...
class LineQRMissionNode(Node):
    ...
    def __init__(self, control_frequency: float = 20.0):
        super().__init__('line_qr_mission_node')
        self.get_logger().info("[LineQR] Initializing Mission Node.")
        ...
        # Publishers
        self.mission_pub = self.create_publisher(MissionStatus, '/mission_3', 10)
        ...
        qr_qos = QoSProfile(depth=10)
        qr_qos.durability = DurabilityPolicy.TRANSIENT_LOCAL
        self.qr_pose_pub = self.create_publisher(PoseStamped, '/qr_obstacle_pose', qr_qos)

        # [Trigger Subscriber]
        # /mission_2 토픽 구독
        self.mission_2_sub = self.create_subscription(
            MissionStatus, '/mission_2', self.mission_2_callback, 10
        )
        ...
        self.get_logger().info("[LineQR] Ready. Waiting for Trigger.")
    ...
    def mission_2_callback(self, msg):
        ...
        if msg.status == 2:
            if not self.mission_start_trigger:
                self.get_logger().info("[LineQR] Trigger Received: Mission 2 Completed.")
    ...
    def _handle_move_forward_1m(self, current_pose):
        if self.initial_move_target is None:
            self.initial_move_start_time = self._now()
            ...
            self.get_logger().info(f"[MOVE] Moving Forward 1m to ({tx:.2f}, {ty:.2f})")
        ...
        if dist < self.POS_TOLERANCE_M:
            self.get_logger().info("[MOVE] 1m Forward Complete.")
            self.state = MissionState.INIT_SCAN_QR
        elif self._elapsed(self.initial_move_start_time) > 15.0:
             self.get_logger().warn("[MOVE] 1m Move Timeout. Force Next State.")
             self.state = MissionState.INIT_SCAN_QR
    ...
    def _handle_scanning_qr(self, current_pose):
        ...
        elapsed = self._elapsed(self.scan_start_time)
        ...
        if abs(yaw_error) < self.YAW_TOLERANCE_RAD:
            ...
            self.get_logger().warn("[SCAN] Timeout rotating to angle. Skipping.")
            self.current_scan_idx += 1
            self.scan_start_time = None
    ...
    def _handle_move_to_hardcoded(self, current_pose):
        ...
        if self.fallback_move_start_time is None:
            self.fallback_move_start_time = self._now()
            self.get_logger().info(
                f"[FALLBACK] Moving to Approach Point: ({target_x:.2f}, {target_y:.2f})"
            )
        ...
        is_timeout = self._elapsed(self.fallback_move_start_time) > 20.0

        if dist < self.POS_TOLERANCE_M or is_timeout:
            if is_timeout:
                self.get_logger().warn("[FALLBACK] Timeout.")
            else:
                self.get_logger().info("[FALLBACK] Arrived at Approach Point.")
    ...
    def _handle_align_qr(self, current_pose):
        ...
        if not objects:
            self.get_logger().warn("[ALIGN] Lost QR. Mission Aborted.")
            self.state = MissionState.MISSION_COMPLETE 
            return
        ...
        if abs(error_pixel) < self.VISION_ALIGN_THRESHOLD_PIXEL:
            if self.align_start_time is None:
                ...
                self.get_logger().info("[ALIGN] QR Centered. Stabilizing...")

            if self._elapsed(self.align_start_time) > self.VISION_ALIGN_STABLE_TIME:
                self.get_logger().info("[ALIGN] QR Align Complete. Mission Complete.")
                self.state = MissionState.MISSION_COMPLETE
        else:
            self.align_start_time = None
    ...
    def run(self):
        ...
        # =========================================================
        self.get_logger().info("[LineQR] Waiting for /mission_2 completion Trigger...")
        while rclpy.ok():
            self.mission_pub.publish(self.mission_msg)
            
            if self.mission_start_trigger:
                self.get_logger().info("[LineQR] Mission 3 START Triggered!")
                break
            self.rate.sleep()
        # =========================================================
        ...
        # =========================================================
        # self.get_logger().info("[LineQR] Waiting for Enter input...")
        # while rclpy.ok():
        #     self.mission_pub.publish(self.mission_msg)
        #     self.get_logger().warn("!!! [Mission 3] Press Enter to Start !!!")
        #     try:
        ...
        self.get_logger().info("[LineQR] Started!")
        ...
        while rclpy.ok():
            ...
            if self.state == MissionState.MOVE_FORWARD_1M:
                ...
                if self.mission_msg.status != MissionStatus.COMPLETED:
                    ...
                    self.get_logger().info("[LineQR] Mission Complete / Aborted.")
