...
class HydrophoneMissionNode(Node):
    # --- Constants (변경 없음) ---
    ...
    BOUND_Y_MIN = 0.0
    BOUND_Y_MAX = 4.0
    MAX_SCAN_CYCLES = 5  # Initial scan + additional retries

    def __init__(self, control_frequency: float = 20.0):
        super().__init__('hydrophone_mission_node')
        self.get_logger().info("[Hydrophone] Initializing Mission 4 Node (Relentless Pursuer Mode).")
        ...
        if _HAVE_HYDRO:
            self.hydro_sub = self.create_subscription(HydrophoneData, '/hydrophone/data', self._hydro_callback, 10)
        else:
            self.get_logger().error("HydrophoneData msg not available; hydrophone mission disabled.")
            self.hydro_sub = None
        self.mission_pub = self.create_publisher(MissionStatus, '/mission_4', 10)
        self.hydro_pose_pub = self.create_publisher(PoseStamped, '/hydrophone_obstacle_pose', 10)

        self.stop_sub = self.create_subscription(Bool, '/mission_4/stop_trigger', self.stop_trigger_callback, 10)
        self.mission_3_sub = self.create_subscription(MissionStatus, '/mission_3', self.mission_3_callback, 10)
        ...
        self.latest_freq, self.latest_db, self.last_msg_time = 0.0, 0.0, rclpy.time.Time()
        self.last_sent_setpoint, self.last_sent_time = None, rclpy.time.Time()
    ...
    def _hydro_callback(self, msg):
        self.latest_freq, self.latest_db, self.last_msg_time = msg.frequency, msg.decibels, self._now()

    def _generate_scan_angles(self, is_wide_scan, center_yaw=0.0):
        self.scan_angles_rad = []
        if is_wide_scan:
            self.get_logger().info("[Hydrophone] Generating WIDE scan angles (360 deg).")
            start_deg, end_deg = -180, 180
        else:
            center_deg = math.degrees(self.wrap_angle(center_yaw))
            self.get_logger().info(
                f"[Hydrophone] Generating NARROW scan angles (+-90 deg around {center_deg:.1f} deg)."
            )
    ...
    def _send_absolute_setpoint(self, x, y, z, yaw):
        current = (x, y, z, yaw)
        now = self._now()
        if self.last_sent_setpoint != current or self._elapsed(self.last_sent_time) > 1.0:
    ...
    def _measure_signal_quality(self) -> dict:
        ...
        for _ in range(self.MOVING_AVG_SAMPLES):
            curr_db, curr_freq = (self.latest_db, self.latest_freq) if self._elapsed(self.last_msg_time) <= 1.0 else (0.0, 0.0)
            db_samples.append(curr_db)
    ...
    def _handle_move_to_search_point(self, pose):
        ...
        if dist < self.DIST_TOLERANCE or self._elapsed(self.move_start_time) > self.XY_TIMEOUT:
    ...
    def _handle_scan_yaw(self, pose):
        if self.current_scan_idx < len(self.scan_angles_rad):
            ...
            if self.move_start_time is None: self.move_start_time = self._now()
            if abs(self.wrap_angle(target_yaw - pose['yaw'])) < self.YAW_TOLERANCE_RAD or self._elapsed(self.move_start_time) > self.YAW_TIMEOUT:
                if self.stabilize_start_time is None: self.stabilize_start_time = self._now()
                if self._elapsed(self.stabilize_start_time) >= self.STABILIZE_TIME:
            ...
            if not sorted_candidates:
                ...
                self.get_logger().warn(
                    f"Scan failed. No valid signal. Failures: {self.detection_failure_count}"
                )
                if self.detection_failure_count > self.MAX_RETRY_COUNT:
                    self.get_logger().error(
                        "Relentless pursuit failed. No signal found after multiple rescans. Moving to failsafe point."
                    )
                    self.state = MissionState.MOVE_TO_FAILSAFE_POINT
                    self.move_start_time = None
                    return
                self.get_logger().info("Retrying with a 360-degree scan at the current location.")
            ...
            if self.scan_iteration > 0 and score < (self.prev_best_score - self.SCORE_DROP_THRESHOLD):
                self.get_logger().warn(
                    f"Signal drop! Score: {self.prev_best_score:.2f} -> {score:.2f}. Initiating 360 rescan."
                )
    ...
    def _handle_align_best_angle(self, pose):
        ...
        is_timeout = self._elapsed(self.move_start_time) > self.YAW_TIMEOUT

        if is_aligned or is_timeout:
            if not self.stabilize_start_time: self.stabilize_start_time = self._now()
            if self._elapsed(self.stabilize_start_time) > 1.0:
                quality = self._measure_signal_quality()
                score_increase = quality['score'] - self.prev_best_score
                self.get_logger().info(
                    f"Aligned. Quality check: Score Rise={score_increase:.2f}, Freq_std={quality['freq_std']:.2f}"
                )

                if self.scan_iteration > 0 and score_increase > self.SCORE_RISE_THRESHOLD and quality['freq_std'] < self.FREQ_STD_THRESHOLD:
                    self.get_logger().warn(
                        "!!! High score rise and signal quality detected! Switching to Phase 2. !!!"
                    )
    ...
    def _handle_precision_search_start(self, pose):
        self.get_logger().info("--- Starting Precision Search Phase ---")
    ...
    def _handle_precision_search_lateral(self, pose):
        ...
        if dist_to_target < self.DIST_TOLERANCE or self._elapsed(self.move_start_time) > self.XY_TIMEOUT:
            self.move_start_time = None
    ...
    def _handle_precision_search_forward(self, pose):
        ...
        if dist_to_target < self.DIST_TOLERANCE or self._elapsed(self.move_start_time) > self.XY_TIMEOUT:
            ...
            if self.precision_sub_state == 'move_forward': self.precision_sub_state = 'return_to_center_3'
                ...
                self.get_logger().info(f"Precision search complete. Final target: {self.final_target_pose}")
    ...
    def _handle_station_keeping(self, pose):
        ...
        if self.mission_msg.status != MissionStatus.COMPLETED:
            self.mission_msg.status = MissionStatus.COMPLETED
            self.get_logger().info("[Hydrophone] Mission Completed or Stopped. Station Keeping.")
    ...
    def _handle_move_to_failsafe_point(self, pose):
        ...
        if self.move_start_time is None: self.move_start_time = self._now()
        if math.hypot(pose['x']-tx, pose['y']-ty) < self.DIST_TOLERANCE or self._elapsed(self.move_start_time) > self.XY_TIMEOUT:
            self.state = MissionState.STATION_KEEPING
    ...
    def run(self):
        ...
        # =========================================================
        self.get_logger().info("[Hydrophone] Waiting for /mission_3 completion Trigger...")
        while rclpy.ok():
            ...
            if hasattr(self, 'mission_start_trigger') and self.mission_start_trigger:
                self.get_logger().info("[Hydrophone] Mission 4 START Triggered!")
                break
            self.rate.sleep()
        # =========================================================
        ...
        # =========================================================
        # self.get_logger().info("[Hydrophone] Waiting for Enter input...")
        # while rclpy.ok():
        #     self.mission_pub.publish(self.mission_msg) # READY 상태 계속 발행
        #     self.get_logger().warn("!!! [Mission 4] Press Enter to Start (Hydrophone Search) !!!")
        #     try:
        ...
        self.get_logger().info("[Hydrophone] Mission 4 Started!")
        self.mission_msg.status = MissionStatus.RUNNING # 상태를 RUNNING으로 전환
        self.state = MissionState.MOVE_TO_SEARCH_POINT
        
        while rclpy.ok() and self.state != MissionState.STATION_KEEPING:
