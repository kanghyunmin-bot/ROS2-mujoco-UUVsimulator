...
class ObjectDetector:
    ...
    def estimate_position(self, 
        ...
        """
        if len(data) != 3:
            rospy.logerr("[ObjectDetector] 입력 데이터는 정확히 3개의 관측점이어야 합니다.")
            return None
        ...
        if intersections.count(None) >= 2:
            rospy.logwarn("[ObjectDetector] 추정 실패: 두 쌍 이상의 관측선이 평행합니다.")
            return None
        ...
        if len(valid_points) < 3:
             rospy.logwarn("[ObjectDetector] 추정 실패: 유효한 교점이 3개 미만입니다.")
             return None
        ...
        if circle_result is None:
            rospy.logwarn("[ObjectDetector] 추정 실패: 3개 교점이 일직선상에 있습니다 (Degenerate case).")
        ...
        if diameter <= self.MAX_DIAMETER:
            ...
            rospy.loginfo(f"[ObjectDetector] 추정 성공: 직경 {diameter:.3f}m < 허용 {self.MAX_DIAMETER}m. 중심 반환.")
            return center
        else:
            # 허용 오차 초과: 추정 실패 (관측 데이터 불일치)
            rospy.logwarn(f"[ObjectDetector] 추정 실패: 직경 {diameter:.3f}m > 허용 {self.MAX_DIAMETER}m. 데이터 불일치.")
            return None
