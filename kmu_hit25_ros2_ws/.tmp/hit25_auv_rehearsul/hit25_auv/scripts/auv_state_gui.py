...
try:
    ...
    print("MAVROS msg not found. Please install ros-noetic-mavros-msgs")
    MavrosState = None

# ── 사용자 설정 ──────────────────────────────────────
# 그래프 설정
MAX_POINTS = 100  # 그래프에 표시할 데이터 포인트 수

# 배터리 셀 추정 설정 (BMS에서 %가 안 넘어올 경우 사용)
V_PER_CELL_FULL = 4.2
...
class AUVStateGUI:
    def __init__(self):
        ...
        # ROS Subscriber 설정
        rospy.Subscriber('/battery', BatteryState, self.batt_cb)
        if MavrosState:
            rospy.Subscriber('/mavros/state', MavrosState, self.state_cb)
        else:
            rospy.logwarn("MAVROS library not found, /mavros/state will not be displayed correctly.")
    ...
    def init_plot(self):
        ...
        # 레이아웃 설정 (GridSpec 사용)
        # 0행: MAVROS 상태, 1행: 배터리 텍스트, 2행: 그래프(전압/전류)
        gs = gridspec.GridSpec(3, 2, height_ratios=[1, 1, 2])

        # 1. MAVROS State 패널 (상단 전체)
    ...
    def batt_cb(self, msg):
        ...
        # 퍼센트 계산 로직 (기존 코드 + 브릿지 값 혼합)
        pct = 0.0
        
        # 1순위: 메시지에 유효한 퍼센트가 있으면 사용 (0~1 사이 값이거나 0~100 사이 값)
        if msg.percentage >= 0:
            # sensor_msgs/BatteryState의 percentage는 보통 0.0~1.0 또는 %단위일 수 있음.
            # 브릿지 코드상 0.0~1.0으로 넘어옴.
            pct = msg.percentage * 100.0 if msg.percentage <= 1.0 else msg.percentage
        
        # 2순위: 메시지 값이 없으면 전압 기반 추정 (기존 코드 로직)
        else:
            with self.lock:
                temp_cells = self.detected_cells

            if temp_cells == 0:
                if curr_v > V_PER_CELL_EMPTY:
        ...
        with self.lock:
            ...
            if self.mavros_state:
                ...
                state_str = (
                    ...
                    f"Guided: {s.guided}     |  Manual Input: {s.manual_input}\n"
                    f"Mode: {s.mode}\n"
                    f"System Status: {s.system_status}"
                )
                
                # Armed 상태에 따라 색상 변경 (시각적 효과)
            ...
            if self.batt_msg:
                ...
                # 배터리 잔량에 따라 색상 변경
                if p < 20: self.text_batt.set_color('red')
                elif p < 50: self.text_batt.set_color('orange')
                else: self.text_batt.set_color('green')
...
def main():
    rospy.init_node('auv_state_gui', anonymous=True)
    gui = AUVStateGUI()

    # ROS Spin을 별도 스레드로 실행
    t = threading.Thread(target=rospy.spin)
    t.daemon = True
    t.start()

    # Matplotlib 애니메이션 실행 (interval=200ms)
    ani = animation.FuncAnimation(gui.fig, gui.update_gui, interval=200)
    plt.show()
