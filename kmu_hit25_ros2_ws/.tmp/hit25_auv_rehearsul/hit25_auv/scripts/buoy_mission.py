...
class BuoyMissionNode:
    ...
    def __init__(self, control_frequency: float = 20.0):
        rospy.init_node('buoy_mission_node', anonymous=True)
        rospy.loginfo("[BuoyMission] Initializing.")
        ...
        # Publishers
        self.mission_pub = rospy.Publisher('/mission_1', MissionStatus, queue_size=10)
        ...
        # 시각화 퍼블리셔
        self.buoy_pose_pub = rospy.Publisher(
            '/buoy_obstacle_pose', PoseStamped, queue_size=10
        )
        self.path_viz_pub = rospy.Publisher(
            '/buoy_planned_path', Path, queue_size=1, latch=True
        )
        ...
        rospy.loginfo("[BuoyMission] Ready. Waiting for trigger.")
    ...
    def _send_smart_setpoint(self, x, y, z, yaw):
        ...
        if self.last_sent_setpoint != current_target:
            should_send = True
        elif (now - self.last_sent_time).to_sec() > 1.0:
            should_send = True
        if should_send:
    ...
    def _handle_yaw_scan(self, current_pose: Dict[str, float]):
        ...
        if buoy_list:
            ...
            if not self._is_duplicate_buoy(current_pose['yaw']):
                rospy.loginfo(f"[VISION] New Buoy Found! -> ALIGN_VISION")
                self.pid.reset() 
                self.state = MissionState.BUOY_ALIGN_VISION 
                self.last_pid_time = rospy.Time.now() - rospy.Duration(self.VISION_PID_PERIOD)
                self.action_start_time = None 
                return

        if self.current_yaw_idx >= len(self.YAW_SCAN_RADIANS):
            rospy.loginfo(f"[SCAN] P{self.current_point_idx + 1} Scan Complete.")
            if self.current_point_idx + 1 >= len(self.OBSERVATION_POINTS):
                 rospy.loginfo("[COMPLETE] All points scanned! Calculating Path...")
                 self.state = MissionState.BUOY_CALCULATE_PATH 
            else:
                 rospy.loginfo("[SCAN] Moving to next point.")
        ...
        if abs(yaw_error) < self.YAW_TOLERANCE_RAD:
            ...
            rospy.logwarn(f"[SCAN] Timeout! Skipping Yaw.")
            self.current_yaw_idx += 1
            self.action_start_time = None
    ...
    def _handle_align_vision(self, current_pose: Dict[str, float]):
        buoy_list = self.vision.find_objects_by_name('Buoy')
        if not buoy_list:
            rospy.logwarn("[VISION] Buoy lost. Returning to SCAN.")
        ...
        if abs(error_pixel) < self.VISION_ALIGN_THRESHOLD_PIXEL:
            if self.align_start_time is None:
                ...
                rospy.loginfo("[VISION] Aligned. Stabilizing...")
            elapsed = (rospy.Time.now() - self.align_start_time).to_sec()
            if elapsed > self.VISION_ALIGN_STABLE_TIME:
                self._record_buoy_data(current_pose)
                rospy.loginfo("[VISION] Recorded. Returning to SCAN.")
                self.state = MissionState.BUOY_YAW_SCAN
                self.align_start_time = None 
        else:
            self.align_start_time = None
    ...
    def _record_buoy_data(self, current_pose):
        ...
        rospy.loginfo(f"   [RECORD] Estimated Buoy Pos: ({buoy_x:.2f}, {buoy_y:.2f})")
    ...
    def _handle_calculate_path(self, current_pose: Dict[str, float]):
        ...
        for i in range(3):
            if i < len(self.recorded_buoys):
                ...
                rospy.loginfo(f"Buoy {i+1}: Detected at {pos}")
            else:
                pos = self.HARDCODED_BUOY_POSITIONS[i]
                rospy.logwarn(f"Buoy {i+1}: Not Detected -> Using Hardcoded {pos}")
        ...
        if path:
            ...
            self.path_idx = 0
            rospy.loginfo(f"Path Generation Success! Waypoints: {len(path)}")
            self._publish_generated_path(path)
            self._publish_final_buoy_poses()
            self.state = MissionState.BUOY_PATH_FOLLOWING
        else:
            rospy.logerr("Path Generation Failed! Mission Abort.")
    ...
    def _publish_generated_path(self, path_points):
        ...
        rospy.loginfo(f"[PATH] Published full trajectory to /buoy_planned_path (Length: {len(path_msg.poses)})")
    ...
    def _handle_path_following(self, current_pose: Dict[str, float]):
        ...
        if self.path_idx >= len(self.generated_path):
            rospy.loginfo("[PATH] End of Path Reached.")
            self.state = MissionState.MISSION_COMPLETE
            return

        # ★ [추가] 타임아웃 체크 (40초 경과 시 강제 종료)
        elapsed_total = (rospy.Time.now() - self.path_start_time).to_sec()
        if elapsed_total > self.TIMEOUT_PATH_TOTAL:
            rospy.logwarn(f"[PATH] Total Timeout ({self.TIMEOUT_PATH_TOTAL}s) Reached! Forcing Mission Complete.")
            self.state = MissionState.MISSION_COMPLETE
            return
    ...
    def run(self):
        self.mission_msg.status = MissionStatus.READY
        rospy.loginfo("[BuoyMission] Waiting for Enter input... (/mission_1 = 0)")
        
        while not rospy.is_shutdown():
            self.mission_pub.publish(self.mission_msg)
            self.debug_pub.publish(self.debug_msg)
            rospy.logwarn_once("!!! [Press Enter to Start] !!!")
            try:
        ...
        rospy.loginfo("[BuoyMission] Started! (/mission_1 = 1)")
        ...
        while not rospy.is_shutdown():
            ...
            if self.state == MissionState.NONE:
                ...
                self.current_yaw_idx = 0
                self.state = MissionState.BUOY_MOVE_TO_POINT
                rospy.loginfo(f"[INIT] Moving to P1.")
                ...
                if self.mission_msg.status != MissionStatus.COMPLETED:
                    ...
                    rospy.loginfo(f"[BuoyMission] Mission Complete! (/mission_1 = 2)")
                    rospy.loginfo(f"[INFO] Stopping /auv_setpoint publishing to release control.")
...
if __name__ == "__main__":
    try:
        try:
            ...
            rospy.logerr("MissionStatus.msg or YawControlDebug.msg not found!")
            sys.exit(1)
        
        controller = BuoyMissionNode(control_frequency=20.0)
        controller.run()
