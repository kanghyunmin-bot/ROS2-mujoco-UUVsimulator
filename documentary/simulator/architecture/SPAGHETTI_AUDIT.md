...
Scope: active runtime code under `sim/current` only.  The backing
...
resolve the simulator through `sim/current` by default.  Missing
...
Closed-loop contract audit now resolves `sim/current` first before
...
setup and closed-loop audit paths resolve `sim/current` first and treat
...
backing directory.  Launching through `sim/current` still passes source
...
setup, and validation paths must be checked through `sim/current`.
...
Runtime freshness is now an audited contract: `sim/current` must resolve
...
`sim/current -> v2.2`, local `HEAD` equals `origin/uuv_sim` at
...
`sim/current/start_sitl_mujoco_mj311.sh` path used by manual tests.
...
source freshness is judged by `sim/current`,
...
- Active runtime freshness was rechecked after this pass.  `sim/current`
...
- `start_docker_sitl_mujoco_mj311.sh`: GUI/Docker direct-start now runs the
  ...
  first proving that `sim/current` and `origin/uuv_sim` are current.
...
- Runtime identity confusion: `sim/current` is the active runtime, while
