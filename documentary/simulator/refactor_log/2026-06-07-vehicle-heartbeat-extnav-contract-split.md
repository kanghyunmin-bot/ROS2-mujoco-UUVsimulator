...
python3 -m compileall -q \
  sim/current/bridge/sitl_vehicle_heartbeat.py \
  sim/current/bridge/sitl_vehicle_heartbeat_filter.py \
  sim/current/bridge/sitl_vehicle_state.py \
  sim/current/bridge/sitl_vehicle_command_ack.py \
  sim/current/bridge/sitl_command_targets.py \
  sim/current/bridge/sitl_commanding.py

sitl_vehicle_heartbeat_split_smoke PASS

python3 -m compileall -q \
  sim/current/bridge/sitl_external_nav_cache_contract.py \
  sim/current/bridge/sitl_external_nav_cache.py \
  sim/current/bridge/sitl_external_nav_contract.py \
