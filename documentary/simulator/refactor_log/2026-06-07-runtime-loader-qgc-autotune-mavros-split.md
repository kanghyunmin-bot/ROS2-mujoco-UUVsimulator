...
Continue reducing active-runtime coupling under `sim/current` while
...
validation paths must use `sim/current` plus the freshness preflight, not
...
/Users/kanghyunmin/miniconda3/envs/ros2_h311/bin/python -m compileall -q \
  sim/current/bridge/ros2_publish_builder_mavros.py \
  sim/current/bridge/ros2_publish_mavros_cache.py \
  sim/current/bridge/ros2_publish_builders.py
