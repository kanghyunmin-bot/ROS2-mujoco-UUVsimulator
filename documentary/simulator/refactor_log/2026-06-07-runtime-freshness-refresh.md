...
- Runtime freshness now compares `HEAD` with `origin/uuv_sim`, verifies
  `sim/current`, and refreshes `uuv_mujoco/RUNTIME_VERSION.json` only when
...
`sim/current` plus the recorded `origin/uuv_sim` source head as the active
runtime contract.
...
python3 sim/current/tools/check_runtime_freshness.py --fetch --refresh-version
zsh -n run_control_gui.sh run_control_gui_ubuntu.sh sim/current/start_sitl_mujoco_mj311.sh
PYTHONPATH=sim/current/tools python3 sim/current/tools/audit_code_contract_sources.py --out-dir /private/tmp/uuv_source_audit_freshness
