...
- Added the active-runtime freshness preflight to
  ...
  backing script now checks `sim/current`, fetches `origin`, compares
...
- Kept `v2.2` as a compatibility backing directory only.  The live runtime
  contract remains `sim/current -> v2.2`.
...
```bash
bash -n sim/current/start_docker_sitl_mujoco_mj311.sh
bash -n sim/current/start_sitl_mujoco_mj311.sh
bash -n sim/run_mujoco.sh
bash -n sim/start_sitl_mujoco.sh
bash -n sim/start_docker_sitl_mujoco.sh
...
python3 -m compileall -q \
  sim/current/tools/roll_stability_metrics.py \
  sim/current/tools/roll_stability_math.py \
  sim/current/tools/roll_stability_pose_metrics.py \
  sim/current/tools/roll_stability_rc_metrics.py \
  sim/current/tools/roll_stability_score.py \
  sim/current/tools/roll_stability_probe_callbacks.py \
  sim/current/tools/roll_stability_probe_sequence.py

PYTHONPATH=sim/current/tools python3 - <<'PY'
import json, math
...
python3 -m compileall -q \
  sim/current/sim/runtime/real_start_payload_status.py \
  sim/current/sim/runtime/real_start_payload_values.py \
  sim/current/sim/runtime/real_start_status_eval.py \
  sim/current/sim/runtime/real_start_payload_builders.py \
  sim/current/sim/runtime/real_start_payload.py \
  sim/current/sim/runtime/real_start_runtime.py
