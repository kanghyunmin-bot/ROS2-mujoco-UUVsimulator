...
python3 -m py_compile \
  sim/current/bridge/ros2_state_estimation.py \
  sim/current/bridge/ros2_state_vertical.py \
  sim/current/bridge/ros2_state_kinematics.py \
  sim/current/bridge/ros2_state_sensors.py \
  sim/current/bridge/ros2_state_setpoint.py

source ./.uuv_mujoco_env.sh && PYTHONPATH=sim/current "$MJ311_PYTHON" <Ros2Bridge binding smoke>

python3 -m py_compile \
  sim/current/tools/actuator_wrench_audit.py \
  sim/current/tools/actuator_wrench_common.py \
  sim/current/tools/actuator_wrench_model.py \
  sim/current/tools/actuator_wrench_calc.py \
  sim/current/tools/actuator_wrench_report.py
