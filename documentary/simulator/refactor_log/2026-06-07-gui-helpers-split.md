...
Scope: active GUI runtime helpers under `sim/current/gui`.
...
python3 -m py_compile \
  sim/current/gui/helpers.py \
  sim/current/gui/gui_math_helpers.py \
  sim/current/gui/gui_rc_helpers.py \
  sim/current/gui/rc_replay_loader.py \
  sim/current/gui/backend_helpers.py \
  sim/current/gui/replay_mixin.py \
  sim/current/gui/ros_process_mixin.py \
  sim/current/gui/autotune_mixin.py \
  sim/current/gui/node_rc_publishers.py \
...
bash -n run_control_gui.sh run_control_gui_ubuntu.sh sim/current/run_control_gui.sh
```
