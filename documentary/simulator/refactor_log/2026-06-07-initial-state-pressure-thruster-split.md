...
MuJoCo thruster actuator update code under `sim/current`.
...
python3 -m compileall -q \
  ...
  sim/current/sim/runtime/initial_state_pose.py \
  sim/current/sim/runtime/initial_state_pose_depth.py \
  sim/current/sim/runtime/initial_state_pose_transforms.py \
  sim/current/sim/runtime/initial_state_hold_capture.py \
  sim/current/bridge/ros2_bridge_config.py \
  sim/current/bridge/ros2_bridge_config_pressure.py \
  sim/current/bridge/ros2_bridge_config_baro.py \
  sim/current/bridge/ros2_bridge_config_imu.py \
  sim/current/bridge/ros2_bridge_config_vertical.py \
  sim/current/sim/runtime/thruster_actuator_runtime.py \
  sim/current/sim/runtime/thruster_actuator_immersion.py \
  sim/current/sim/runtime/thruster_actuator_forces.py \
  sim/current/sim/runtime/thruster_actuator_visuals.py \
  sim/current/sim/runtime/thruster_command_targets.py

PYTHONPATH=sim/current python3 <initial-state, pressure-config, and thruster import smokes>
...
python3 sim/current/tools/refactor_inventory.py \
  --root sim/current --limit 25 --format markdown
```
