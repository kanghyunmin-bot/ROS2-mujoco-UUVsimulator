...
- Ping360 publishes a lightweight on-demand sonar image only when a subscriber
  is present.
- The synthetic hydrophone uses the course pinger buoy site as a 21,164 Hz
  acoustic source by default. `ROS2_UUV_HYDROPHONE_FREQ_HZ=27211` selects the
  other competition frequency without changing the bridge code.
- The normal simulator profile retains the configurable legacy near-frequency
  interferers. Pinger Start instead selects the `rosbag_20260707` profile. That
  profile is calibrated from the stationary July 7 recording: 0.19 rad bounded
  quiet-carrier phase wander, the measured channel-common 21,332.8645 Hz
  nuisance line, a 23.8--24.4 kHz propulsion envelope, and heavy-tailed
  clipping stress. Propulsion noise is gated by the RMS of actual MuJoCo
  actuator controls, so it rises while ALT_HOLD or forward motion drives the
  thrusters and falls away when the actuators are idle.
- At full normalized thruster activity, the profile targets approximately
  0.411 full-scale RMS and 6.9 percent clipped samples, matching the recorded
  armed interval. The quiet receiver floor remains independently configurable.
  `/mujoco/hydrophone/status` reports the active profile, phase setting, and
  current normalized thruster activity.
