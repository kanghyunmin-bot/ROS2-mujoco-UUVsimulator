...
| --- | --- | --- | --- |
| macOS development | native ArduSub SITL | local Python env | `mjpython` when viewer is enabled |
| Ubuntu development | native ArduSub SITL | local Python env | Python with X11/Wayland display |
...
- Ubuntu migration markers in the native launcher and docs,
- launch script executability,
- active runtime alias availability: `sim/current -> v2.2`,
...
replay should pass without either.

For Ubuntu migration, the expected development flow is:

```bash
cd /path/to/uuv_sim/sim/current
python3 tools/check_dev_os_compat.py --headless --target-os ubuntu --strict
