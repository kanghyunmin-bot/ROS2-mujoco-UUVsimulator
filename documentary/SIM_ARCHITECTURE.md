...
이 문서는 `sim/current`와 `rospkg/src`의 실제 코드 상태를 기준으로 한
...
활성 경로는 항상 `sim/current`다. 과거 호환 복사본 `uuv_mujoco/v2.2`는
워크스페이스 정리 과정에서 제거되었으며, `current`는 심볼릭 링크가 아닌 독립
디렉터리다. 이전 alias 설명은 `documentary/archive/LEGACY_RUNTIME_LAYOUT.md`에
기록 목적으로만 보존한다.
...
자세한 topic owner는 `documentary/contracts/REAL_STACK_PARITY.md`를 기준으로 한다.
...
`sim/current/tools`의 검사는 세 단계로 나뉜다.
