...
DEFAULT_MODEL_NAME = "yolo26m_underwater_batch4_last.pt"
...
def create_yolo_buoy_detector() -> "YoloBuoyDetector":
    ...
    mode = os.environ.get("UUV_GUI_YOLO_ENABLE", "auto").strip().lower()
...
class YoloBuoyDetector:
    ...
    def process_rgb(self, rgb: np.ndarray) -> tuple[np.ndarray, dict[str, Any]]:
        ...
        if recently_processed:
            return draw_buoy_detections(rgb, cached), self.status_payload()
        ...
        try:
            ...
            drawn = draw_buoy_detections(rgb, detections)
...
def _resolve_model_path() -> Path:
    ...
    if workspace is not None:
        candidates.append(workspace / "YOLO" / DEFAULT_MODEL_NAME)
    candidates.append(Path(__file__).resolve().parents[3] / "YOLO" / DEFAULT_MODEL_NAME)
    candidates.append(Path.cwd() / "YOLO" / DEFAULT_MODEL_NAME)
