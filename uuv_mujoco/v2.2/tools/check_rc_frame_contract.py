...
def check_gui_rc_input_span_contract() -> None:
    ...
    assert RC_PWM_SPAN == 300.0
    assert axis_to_pwm(1.0) == 1800
    assert axis_to_pwm(-1.0) == 1200
    forced = rc_override_contract()
    assert forced["UUV_GUI_RC_PWM_SPAN"] == "300"
    assert forced["ROS2_UUV_MAVROS_RC_PWM_SPAN"] == "300"
    ...
    assert bridge._mavros_rc_pwm_span == 300.0
