...
- `tools/audit_code_contract_sources.py` now resolves and reports the active
  runtime as `uuv_mujoco/current`, splits source checks from report generation,
...
- `tools/audit_code_contract_source_identity.py` now verifies
  `uuv_mujoco/current -> v2.2`, the active primary runner, and
...
- Root current-runtime launchers now exist at `uuv_mujoco/run_mujoco.sh`,
  `uuv_mujoco/start_sitl_mujoco.sh`,
  `uuv_mujoco/start_docker_sitl_mujoco.sh`, and `uuv_mujoco/reset_sim.sh`.
  They resolve `UUV_MUJOCO_RUNTIME_DIR` or `uuv_mujoco/current`, so normal
...
- Root launch wrappers now run `tools/check_runtime_freshness.py --fetch
  ...
  `uuv_mujoco/current`, and warns when `RUNTIME_VERSION.json` is stale.  This
...
- Dynamic MuJoCo `fluidcoef` support is split into
  ...
  facade.
- `tools/audit_closed_loop_contract.py` now resolves `uuv_mujoco/current`
...
- Active-runtime provenance now lives in `uuv_mujoco/RUNTIME_VERSION.json`;
  setup and closed-loop audit paths resolve `uuv_mujoco/current` first and only
