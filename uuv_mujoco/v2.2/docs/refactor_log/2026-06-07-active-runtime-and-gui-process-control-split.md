...
- Updated setup and closed-loop audit resolution so `uuv_mujoco/current` is the
...
python3 -m py_compile \
  uuv_mujoco/current/gui/control_display_mixin.py \
  uuv_mujoco/current/gui/control_pilot_mixin.py \
  uuv_mujoco/current/gui/control_toggle_mixin.py \
  uuv_mujoco/current/gui/control_draw_mixin.py \
  uuv_mujoco/current/gui/control_feedback_mixin.py \
  uuv_mujoco/current/gui/control_update_mixin.py \
  uuv_mujoco/current/gui/sim_stack_process_mixin.py \
  uuv_mujoco/current/gui/sim_stack_status_mixin.py \
  uuv_mujoco/current/gui/sim_stack_launch_mixin.py \
  uuv_mujoco/current/gui/sim_stack_reset_mixin.py \
  uuv_mujoco/current/tools/audit_closed_loop_contract.py \
  uuv_mujoco/current/tools/dev_os_compat_system.py \
  uuv_control_gui.py
python3 -m json.tool uuv_mujoco/RUNTIME_VERSION.json
python3 uuv_mujoco/current/tools/refactor_inventory.py --limit 15 --format markdown
```
