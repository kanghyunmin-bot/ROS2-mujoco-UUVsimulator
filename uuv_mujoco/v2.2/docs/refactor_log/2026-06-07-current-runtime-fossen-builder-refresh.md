...
`uuv_mujoco/current -> v2.2`, with freshness checked against the active git
branch before launcher execution.

## What changed

- Verified `uuv_mujoco/current -> v2.2` and refreshed
...
- New runtime and validation commands continue to target
  `uuv_mujoco/current`, not historical absolute `v2.2` paths.
