...
- Added `active_runtime_alias_current` to the source-contract audit so
  `uuv_mujoco/current`, the primary runner, and `uuv_mujoco/RUNTIME_VERSION.json`
...
- `tools/refactor_inventory.py --root uuv_mujoco/current --limit 15` no longer
  lists `bridge/sitl_transport_config.py` or `gui/physics_mixin.py` in the top
  hotspot list.
- `PYTHONPATH=uuv_mujoco/current/tools python3
  uuv_mujoco/current/tools/audit_code_contract_sources.py --out-dir
