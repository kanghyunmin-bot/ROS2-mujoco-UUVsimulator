...
- Setup still accepts old zip/package layouts that contain only `v2.2`, but it
  recreates `uuv_mujoco/current` and then runs through that active alias.
- New docs and commands should use `uuv_mujoco/current`; `v2.2` is only a
...
# /Users/kanghyunmin/Desktop/uuv_sim/uuv_mujoco/current
```
