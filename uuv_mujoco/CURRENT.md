...
the active runtime through `uuv_mujoco/current` unless they are referencing
historical evidence paths.

Operational rule:

- Treat `uuv_mujoco/current` as the only live runtime path.
...
entry points.  If `uuv_mujoco/current` is missing, setup should recreate the
...
`UUV_MUJOCO_RUNTIME_DIR` first, and fall back to `uuv_mujoco/current`.
...
```text
uuv_mujoco/current/tools/check_runtime_freshness.py --fetch --refresh-version --warn-only
```
...
checks `uuv_mujoco/current`, records dirty working-tree/runtime evidence, and
...
```text
uuv_mujoco/current/run_urdf_full.py
```
