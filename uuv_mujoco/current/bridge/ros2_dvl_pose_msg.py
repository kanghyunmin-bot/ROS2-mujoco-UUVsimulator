...
from .ros2_math import quat_to_yaw, set_first_attr, set_nested_xyz
...
def _set_dvl_attitude_fields(msg: Any, quat_ros: np.ndarray) -> None:
    if not (hasattr(msg, "roll") or hasattr(msg, "pitch") or hasattr(msg, "yaw")):
        return
    # dvl_msgs variants differ; yaw is the useful shared pose field here.
    set_first_attr(msg, ("yaw",), float(quat_to_yaw(quat_ros)))
    set_first_attr(msg, ("roll",), 0.0)
    set_first_attr(msg, ("pitch",), 0.0)
...
def build_dvldr_msg(
    ...
    if dvldr_msg_type is None:
        return None
    msg = dvldr_msg_type()
    stamp_header(msg, stamp, "odom")
    _set_dvl_position_fields(msg, position_flu)
    _set_dvl_attitude_fields(msg, quat_ros)
    return msg
