...
def send_rc_override(self, pwm_values: list[int]) -> bool:
    ...
    frame, but send it over the active command link.  In Docker/QGC mode the
    servo telemetry link is intentionally isolated from QGC serial0 and can be
    unavailable as a command path; the dedicated serial4 command link is the
