...
(() => {
  ...
  const rlQuick = document.createElement('button');
  rlQuick.textContent = '강화학습 모드';
  document.querySelector('.header-actions').append(rlQuick);
  rlQuick.onclick = () => window.open('http://127.0.0.1:8882/rl', '_blank', 'noopener');
  const autoQuick = document.createElement('button');
  autoQuick.id = 'autoCollectionQuickOpen'; autoQuick.textContent = '자동 수집 모드';
  document.querySelector('.header-actions').append(autoQuick);
  autoQuick.onclick = () => { openCollection(); document.getElementById('autoCollectionPanel').scrollIntoView({block:'center'}); };
