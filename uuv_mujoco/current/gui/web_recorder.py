...
from .recording_partition import recording_partition
...
class WebRecorder:
    ...
    def payload(self):
        with self.lock:
            ...
            if now >= getattr(self, '_counts_refresh_at', 0.0):
                self._collection_counts = collection_counts(getattr(self, 'dataset_base', APP_ROOT / 'outputs/vla-experiments/unselected/datasets'))
            ...
            return {**self.status, 'online': fresh, 'owned': owned, 'running': running,
                    ...
                    'dataset_partition': getattr(self, 'dataset_partition', None),
    ...
    def prepare(self, task, mode):
        ...
        with self.lock:
            ...
            scene = Path(simulation.get('active_scene') or SIM_STACK_DIR / 'scenes/research_pool_slam_scene.xml')
            control_profile = (
                'yaw-stable-20260920-v1'
                if simulation.get('active_preset_id') == 'research_pool_yaw_stable' else None
            )
            dataset_base, partition = recording_partition(
                APP_ROOT, scene, camera, control_profile=control_profile,
            )
            self.dataset_base = dataset_base
            self.dataset_partition = partition
            root = dataset_base / self.session
