...
from sim.startup_alignment import alignment_required


def create_initial_depth_hold_state(*, args: Any, initial_bar30: InitialBar30Depth) -> dict:
    startup_hold = bool(getattr(args, "sitl", False) and alignment_required())
    state = InitialDepthHoldState.create(
        active=bool(args.hold_initial_depth_until_release or startup_hold),
    ...
    state.startup_alignment_required = startup_hold
    return state
...
def resolve_initial_real_start_policy(
    *,
    ...
    return InitialRealStartPolicy(
        auto_release=bool(
            args.sitl
            and (alignment_required() or env_flag(
