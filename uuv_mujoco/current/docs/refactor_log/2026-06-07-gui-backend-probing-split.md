...
Scope: active GUI backend detection code under `uuv_mujoco/current/gui`.
...
python3 -m compileall -q \
  ...
  uuv_mujoco/current/gui/node_backend_counts.py \
  uuv_mujoco/current/gui/node_backend_layout.py \
  uuv_mujoco/current/gui/node_backend_selection.py \
...
python3 uuv_mujoco/current/tools/refactor_inventory.py \
  --root uuv_mujoco/current --limit 20 --format markdown
