...
`uuv_mujoco/current` with the physical backing directory still named
...
python3 -m py_compile \
  uuv_mujoco/current/bridge/ros2_bridge.py \
  uuv_mujoco/current/bridge/ros2_bridge_runtime_setup.py \
  uuv_mujoco/current/bridge/ros2_bridge_sensor_setup.py \
  uuv_mujoco/current/bridge/ros2_bridge_method_bindings.py \
  uuv_mujoco/current/sim/physics/dynamic_fluidcoef_runtime.py \
  uuv_mujoco/current/sim/physics/dynamic_fluidcoef_runtime_config.py

source ./.uuv_mujoco_env.sh
PYTHONPATH="uuv_mujoco/current:${PYTHONPATH:-}" "${MJ311_PYTHON:-python3}" - <<'PY'
