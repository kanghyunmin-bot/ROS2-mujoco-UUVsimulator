...
Scope: active runtime transport code under `uuv_mujoco/current/bridge` and
...
python3 -m py_compile \
  uuv_mujoco/current/bridge/sitl_json_sensor_runtime.py \
  uuv_mujoco/current/bridge/sitl_json_payload.py \
  uuv_mujoco/current/bridge/sitl_json_sender.py \
  uuv_mujoco/current/bridge/sitl_json_replay_reply.py \
  uuv_mujoco/current/bridge/sitl_json_sensor_send.py \
  uuv_mujoco/current/bridge/sitl_transport.py \
  uuv_mujoco/current/tools/audit_code_contract_paths.py \
  uuv_mujoco/current/tools/audit_code_contract_runtime_checks.py
