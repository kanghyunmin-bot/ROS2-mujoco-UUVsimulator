...
def _roll_row(
    *,
    ...
    return {
        ...
        "rcin_onset_delay_s": rcin_onset_delay_s,
...
def main() -> int:
    ...
    slow_rcin_echo = _single_check(
        _health_for(
            _roll_row(
                rcin_delta=75.0,
                rcout_delta=75.0,
                gyro_peak=0.05,
                rcin_onset_delay_s=0.80,
            )
        )
    )
    assert slow_rcin_echo["severity"] == "warn", slow_rcin_echo
    assert slow_rcin_echo["flags"] == ["slow_rcin_command_echo"], slow_rcin_echo
