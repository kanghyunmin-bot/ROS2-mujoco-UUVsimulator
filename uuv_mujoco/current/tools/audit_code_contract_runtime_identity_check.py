...
def build_active_runtime_alias_check() -> tuple[Check, dict[str, str]]:
    ...
    check = Check(
        ...
        title="Active runtime resolves through uuv_mujoco/current",
        conclusion=(
            "Live launch, GUI, Docker, setup, and validation paths must resolve the active runtime "
            "through uuv_mujoco/current. The v2.2 directory name is only the compatibility backing "
