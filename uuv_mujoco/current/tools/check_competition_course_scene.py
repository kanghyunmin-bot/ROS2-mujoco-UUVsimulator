...
def check_fixed_buoys(root: ET.Element, bodies: dict[str, ET.Element], failures: list[str]) -> None:
    ...
    for base_name in bases:
        ...
        if rod is not None:
            ...
            if len(endpoints) == 6:
                ...
                require(
                    ...
                    and math.isclose(endpoints[5], 2.195, abs_tol=1e-6),
        ...
        if pvc is not None:
            ...
            require(len(rgba) == 4 and min(rgba[:3]) >= 0.96 and rgba[3] == 1.0, f"{prefix} PVC must be opaque white", failures)
            require(len(fromto) == 6, f"{prefix} PVC stick must use explicit endpoints", failures)
            if len(fromto) == 6:
                require(0.035 <= abs(fromto[5] - fromto[2]) <= 0.13, f"{prefix} PVC stick length is outside the compact buoy assembly", failures)
        ...
        if top_jig is not None and attach_site is not None:
            ...
            require(
                0.38 <= assembly_height <= 0.46,
                f"{prefix} buoy/stick/jig assembly must be about 400mm, got {assembly_height:.3f}m",
                failures,
            )
        moving_jig_name = (
