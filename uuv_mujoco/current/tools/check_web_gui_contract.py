...
def main() -> int:
    ...
    mission_core_cpp = ROOT / "missionFSM" / "ground_truth_buoy_controller_core.cpp"
    mission_split_cpp = ROOT / "missionFSM" / "ground_truth_buoy_mission_fsm.cpp"
    mission_fsm_cpp = ROOT / "missionFSM" / "ground_truth_buoy_fsm.cpp"
    mission_fsm_cpp_contract = ROOT / "missionFSM" / "check_ground_truth_buoy_fsm_cpp.cpp"
    ...
    require_text(
        ...
        "sim pinger start must use the same scan/select launch as the physical vehicle",
    )
    reject_text(
        web_process_manager,
    ...
    require_text(
        ...
        "sim parity must not use a hidden simulator-only range calibration",
    )
    require_text(
        web_process_manager,
        '"auto_select_top:=true"',
        "headless GUI launch must use the physical FFT selector and only auto-pick a qualified peak",
    )
    require_text(
        web_process_manager,
        '"scan_monitor_s:=10.0"',
        "sim and real must use the same ten-second scan",
    )
    require_text(
        web_process_manager,
    ...
    require_text(mission_core_cpp, "ground_truth_buoy_fsm.cpp", "missionFSM core C++ boundary must share canonical implementation")
    require_text(mission_split_cpp, "ground_truth_buoy_fsm.cpp", "missionFSM C++ split boundary must share canonical implementation")
    require_text(mission_fsm_cpp, "/mujoco/course_buoys/status", "C++ mission FSM must subscribe to live MuJoCo buoy status")
    require_text(mission_fsm_cpp, "robot_state", "C++ mission FSM must publish robot mission state")
    require_text(mission_fsm_cpp, "/mavros/rc/override", "C++ mission FSM must publish RC override commands")
    require_text(mission_fsm_cpp, "MISSION_COMPLETE", "C++ mission FSM must expose mission completion state")
    require_text(mission_fsm_cpp, "SCORE_ZONE_TRANSIT", "C++ mission FSM must drive scored buoys to the score zone")
    require_text(mission_fsm_cpp_contract, "commit timeout must not mark pinger failed", "missionFSM C++ contract must guard pinger retry policy")
