...
def _check_runtime_scripts(results: list[CheckResult]) -> None:
    scripts = [
        ...
        ROOT / "start_docker_sitl_mujoco_mj311.sh",
...
def _check_ardupilot_tree(results: list[CheckResult]) -> None:
    ardupilot = WORKSPACE / "ardupilot"
    results.append(
        CheckResult(
            "ardupilot_tree",
            "pass" if ardupilot.exists() else "warn",
            str(ardupilot) if ardupilot.exists() else "missing; Docker build/runtime will need clone",
        )
    )
