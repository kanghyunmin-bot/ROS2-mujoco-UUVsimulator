...
DEFAULT_OUT = Path("document/docsource/closed_loop_april1_rc_replay")
PWM_CENTER = 1500.0
...
class ClosedLoopRcReplayer(Node):
    ...
    def replay(self, seq: RcSequence, *, rate_scale: float, mode_sequence: ModeSequence | None = None) -> None:
        ...
        for sample_t, channels in zip(seq.t, seq.channels):
            ...
            if mode_sequence is not None:
                while next_mode_idx < len(mode_sequence.events):
                    ...
                    next_mode_idx += 1
            self.publish_phase("closed_loop_replay")
            self.publish_channels(channels)
            rclpy.spin_once(self, timeout_sec=0.0)
        self.events.append({"t": self.elapsed(), "type": "replay_end"})
...
def main() -> None:
    ...
    parser.add_argument(
        "--command-source",
        choices=("rc-override", "joy-node"),
        default="joy-node",
        help="Replay recorded /mavros/rc/override directly, or rebuild it from /joy like the real joy node.",
    )
    ...
    if args.command_source == "joy-node":
        ...
        seq = load_rc_sequence(
            ...
            duration_s=args.duration_s,
            max_publish_hz=args.max_publish_hz,
            fill_neutral_first8=args.fill_neutral_first8,
        )
    mode_sequence = (
        load_mode_sequence(
            args.bag,
    ...
    try:
        ...
        if args.release_sim_initial_depth_hold:
        ...
        node.replay(seq, rate_scale=args.rate_scale, mode_sequence=None if args.no_mode else mode_sequence)
