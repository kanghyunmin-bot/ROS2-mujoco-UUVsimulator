...
- sim_csv: `/Users/kanghyunmin/Desktop/uuv_sim/UUV-HAN/outputs/plant_contract_structure_v65_20260615/real_rcou_startvel_local_xy_dvl_z_vhist2s_10s/sim_sensor_replay.csv`
...
| dvl_vel_y | dvl_y_mps | 0.134 | `False` |
| dvl_vel_z | dvl_z_mps | 0.138 | `False` |
...
| imu_gyro_y | gyro_y_radps | -0.037 | `False` |
| imu_gyro_z | gyro_z_radps | 0.743 | `False` |
...
| imu_data | 20.000 | 17.857 | 0.893 | 201 | 201 |
...
- yaw_delta_deg real/sim/ratio: `-181.870` / `-171.870` / `0.945`
- gyro_z_integral_deg real/sim/ratio: `-181.762` / `-171.759` / `0.945`

### Windowed Yaw Drift

| window_s | real_mean_r | sim_mean_r | mean_err_r | real_int_deg | sim_int_deg | deficit_deg | ratio |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 0.00-10.00 | -0.3159 | -0.2996 | 0.0163 | -181.8 | -171.8 | 10.0 | 0.945 |

## Thruster Debug

- available: `True`; active_rows: `214`
- thr_torque_body_z mean/rms/min/max Nm: `-2.109` / `3.190` / `-9.587` / `0.838`
- ang_vel_body_z mean/rms/min/max rad/s: `-0.308` / `0.433` / `-0.973` / `0.133`
...
| depth_pose_m | 0.013 | 0.002 | -0.006 | -2.950 |
| imu_roll_deg | 0.357 | 0.332 | 0.239 | 0.200 |
| imu_pitch_deg | 0.864 | -0.178 | -0.558 | 3.450 |
| imu_yaw_rel_deg | 24.325 | 0.987 | 21.124 | 0.200 |
| dvl_x_mps | 0.055 | -0.012 | -0.033 | 2.800 |
| dvl_y_mps | 0.056 | 0.134 | 0.031 | 1.450 |
| dvl_z_mps | 0.024 | 0.138 | 0.021 | 3.000 |
| gyro_x_radps | 0.018 | 0.510 | -0.001 | -4.450 |
| gyro_y_radps | 0.040 | -0.037 | -0.005 | 4.650 |
| gyro_z_radps | 0.250 | 0.743 | 0.016 | -4.400 |
| accel_x_mps2 | 0.163 | 0.148 | -0.059 | -4.350 |
| accel_y_mps2 | 0.270 | 0.551 | 0.208 | -0.250 |
| accel_z_mps2 | 0.067 | 0.031 | 0.021 | 3.150 |
...
| signal | RMSE | corr | bias | lag_s |
| --- | ---: | ---: | ---: | ---: |
| local_depth_m | 0.009 | -0.096 | -0.006 | -2.000 |
| local_x_m | 0.117 | 0.996 | -0.098 | 0.150 |
| local_y_m | 0.261 | 0.781 | 0.127 | 0.800 |
| local_vel_x_mps | 0.048 | 0.908 | 0.003 | 0.450 |
| local_vel_y_mps | 0.096 | 0.971 | 0.063 | 0.150 |
| local_vel_z_mps | 0.015 | 0.077 | -0.011 | 3.050 |
...
| sim | x | -0.041 | 0.074 | -0.248 | 0.208 |
| sim | y | -0.101 | 0.154 | -0.403 | 0.208 |
| sim | z | -0.846 | 0.847 | -0.964 | -0.449 |
...
| sim | x | 200 | 0.003 | 0.967 | -0.000 |
| sim | y | 200 | 0.005 | 0.985 | -0.000 |
| sim | z | 200 | 0.024 | 0.997 | -0.000 |
...
- sim gyro self-axis: identity_corr_rank=`1`, identity_rmse_rank=`1`, identity_rmse_sum=`0.032`, best_rmse_sum=`0.032`, best_rmse_mapping=`['real_x <= +sim_x', 'real_y <= +sim_y', 'real_z <= +sim_z']`
...
| sim_local_surface | x | 0.952 | 0.013 | 0.987 |
| sim_local_surface | y | -0.988 | 0.001 | -0.972 |
| sim_local_surface | z | -0.439 | -0.006 | -0.674 |
| sim_truth_odom | x | 1.004 | -0.001 | 0.988 |
| sim_truth_odom | y | 0.991 | -0.000 | 0.993 |
| sim_truth_odom | z | 1.352 | -0.004 | 0.841 |
...
| sim_local_surface | -0.0014 | 0.0008 | 0.0534 | 0.0048 | 0.0047 | 0.021 |
| sim_truth_odom | -0.0000 | -0.0002 | -0.0086 | 0.0033 | 0.0033 | 0.002 |

## Axis Audit

- dvl_twist: identity_rank=`7`, identity_rmse_rank=`1`, best=`real_x <= -sim_y, real_y <= +sim_z, real_z <= -sim_x`, sumcorr=`0.533`, best_rmse=`real_x <= +sim_x, real_y <= +sim_y, real_z <= +sim_z`, rmse_sum=`0.135`
- imu_gyro: identity_rank=`2`, identity_rmse_rank=`2`, best=`real_x <= +sim_x, real_y <= -sim_y, real_z <= +sim_z`, sumcorr=`1.290`, best_rmse=`real_x <= +sim_x, real_y <= -sim_y, real_z <= +sim_z`, rmse_sum=`0.306`
- imu_accel: identity_rank=`1`, identity_rmse_rank=`1`, best=`real_x <= +sim_x, real_y <= +sim_y, real_z <= +sim_z`, sumcorr=`0.729`, best_rmse=`real_x <= +sim_x, real_y <= +sim_y, real_z <= +sim_z`, rmse_sum=`0.499`
