...
POST_ARM_SETTLE_S="${UUV_POST_ARM_SETTLE_S:-15}"
POST_MODE_SETTLE_S="${UUV_POST_MODE_SETTLE_S:-3}"
...
Options:
  ...
  --post-arm-settle-s SEC       Neutral settle after arming before mode change (default: UUV_POST_ARM_SETTLE_S or 15).
  --post-mode-settle-s SEC      Neutral settle after mode confirmation before depth-hold release (default: UUV_POST_MODE_SETTLE_S or 3).
...
# MAVProxy already owns ArduPilot's single TCP master on 5760.  The dedicated
# command fan-out is SERIAL4/MAVProxy UDP 14661; binding the bridge to TCP 5760
# leaves rc_override_ready false forever and means the supposed golden check
# never arms or tests the plant.
...
if [[ "$START_STACK" -eq 1 ]]; then
  ...
  if [[ "${MODE^^}" == "ALT_HOLD" ]]; then
    # Keep the physical pose fixed while EKF/Bar30 settle and ArduSub captures
    # its ALT_HOLD target.  The axis runner releases this only after the mode
    # transition has been confirmed.
    MUJOCO_ARGS+=(--hold-initial-depth-until-release)
  fi
...
AXIS_CMD=(
  ...
  --post-mode-settle-s "$POST_MODE_SETTLE_S"
  --sample-hz "$SAMPLE_HZ"
  --axes "${AXES[@]}"
)
if [[ "${MODE^^}" == "ALT_HOLD" ]]; then
  AXIS_CMD+=(--release-initial-depth-hold --post-release-neutral-s "$POST_ARM_SETTLE_S")
fi
...
COMPARE_CMD=(
  python3 "${SCRIPT_DIR}/../../analysis/tools/simulator/control_loop_golden_compare.py"
