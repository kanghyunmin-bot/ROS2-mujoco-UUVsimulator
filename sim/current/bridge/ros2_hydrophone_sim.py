...
class HydrophoneConfig:
    ...
    pool_z_min_m: float
    pool_z_max_m: float
    noise_profile: str = "rosbag_20260707"
    pinger_frequency_bias_hz: float = 0.0
    pinger_phase_noise_std_rad: float = 0.0
    pinger_phase_noise_correlation_s: float = 2.0
    rosbag_thruster_activity_reference: float = 0.12
    rosbag_thruster_broadband_rms: float = 0.34
    rosbag_thruster_impulse_probability: float = 0.07
    rosbag_thruster_impulse_amplitude: float = 1.50
    rosbag_thruster_common_fraction: float = 1.0
    rosbag_stationary_tone_frequency_hz: float = 21332.8645
    rosbag_stationary_tone_amplitude: float = 0.021
...
def configure_hydrophone_runtime(bridge: Any) -> None:
    ...
    cfg = HydrophoneConfig(
        ...
        snr_probe_noise_amplitude=float(
            np.clip(
                env_to_float("ROS2_UUV_HYDROPHONE_SNR_PROBE_NOISE_AMPLITUDE", 0.0),
                0.0,
                0.05,
            )
        ),
        max_range_m=float(np.clip(env_to_float("ROS2_UUV_HYDROPHONE_MAX_RANGE_M", 80.0), 0.1, 500.0)),
        interferers_enabled=bool(env_to_int("ROS2_UUV_HYDROPHONE_INTERFERERS_ENABLE", 0)),
        interferer_count=int(np.clip(env_to_int("ROS2_UUV_HYDROPHONE_INTERFERER_COUNT", 0), 0, 64)),
        interferer_thruster_count=int(
            np.clip(env_to_int("ROS2_UUV_HYDROPHONE_INTERFERER_THRUSTER_COUNT", 0), 0, len(THRUSTER_NOISE_SITES))
        ),
        interferer_frequency_span_hz=float(
            np.clip(env_to_float("ROS2_UUV_HYDROPHONE_INTERFERER_FREQ_SPAN_HZ", 470.0), 0.0, 100000.0)
        ),
        interferer_amplitude=float(
            np.clip(env_to_float("ROS2_UUV_HYDROPHONE_INTERFERER_AMPLITUDE", 0.0), 0.0, 0.50)
        ),
        ...
        noise_profile=_env_str("ROS2_UUV_HYDROPHONE_NOISE_PROFILE", "rosbag_20260707").strip().lower(),
        pinger_frequency_bias_hz=float(
            np.clip(env_to_float("ROS2_UUV_HYDROPHONE_FREQUENCY_BIAS_HZ", 0.0), -500.0, 500.0)
        ),
        pinger_phase_noise_std_rad=float(
            np.clip(env_to_float("ROS2_UUV_HYDROPHONE_PHASE_NOISE_STD_RAD", 0.19), 0.0, math.pi)
        ),
        pinger_phase_noise_correlation_s=float(
            np.clip(env_to_float("ROS2_UUV_HYDROPHONE_PHASE_NOISE_CORRELATION_S", 2.0), 0.01, 120.0)
        ),
        rosbag_thruster_activity_reference=float(
            np.clip(env_to_float("ROS2_UUV_HYDROPHONE_ROSBAG_ACTIVITY_REFERENCE", 0.12), 0.001, 1.0)
        ),
        rosbag_thruster_broadband_rms=float(
            np.clip(env_to_float("ROS2_UUV_HYDROPHONE_ROSBAG_BROADBAND_RMS", 0.34), 0.0, 1.0)
        ),
        rosbag_thruster_impulse_probability=float(
            np.clip(env_to_float("ROS2_UUV_HYDROPHONE_ROSBAG_IMPULSE_PROBABILITY", 0.07), 0.0, 0.5)
        ),
        rosbag_thruster_impulse_amplitude=float(
            np.clip(env_to_float("ROS2_UUV_HYDROPHONE_ROSBAG_IMPULSE_AMPLITUDE", 1.50), 0.0, 4.0)
        ),
        rosbag_thruster_common_fraction=float(
            np.clip(env_to_float("ROS2_UUV_HYDROPHONE_ROSBAG_COMMON_FRACTION", 1.0), 0.0, 1.0)
        ),
        rosbag_stationary_tone_frequency_hz=float(
            np.clip(env_to_float("ROS2_UUV_HYDROPHONE_ROSBAG_STATIONARY_TONE_HZ", 21332.8645), 100.0, 48000.0)
        ),
        rosbag_stationary_tone_amplitude=max(
            0.0, env_to_float("ROS2_UUV_HYDROPHONE_ROSBAG_STATIONARY_TONE_AMPLITUDE", 0.021)
        ),
    )
    ...
    bridge._hydrophone_pinger_phase_noise_rad = 0.0
    bridge._hydrophone_last_thruster_activity = 0.0
    ...
    if cfg.enabled:
        print(
            ...
            f"audio={cfg.audio_enabled} noise_profile={cfg.noise_profile} "
            f"interferers={len(bridge._hydrophone_noise_sources)}",
            flush=True,
        )
...
def build_hydrophone_status_msg(bridge, data, _stamp, state):
    ...
    payload = {
        ...
        "pinger_frequency_bias_hz": cfg.pinger_frequency_bias_hz,
        "pinger_phase_noise_std_rad": cfg.pinger_phase_noise_std_rad,
        "rosbag_stationary_tone_hz": cfg.rosbag_stationary_tone_frequency_hz,
        "thruster_activity": float(getattr(bridge, "_hydrophone_last_thruster_activity", 0.0)),
        **measurement,
    }
...
def build_hydrophone_audio_msg(bridge, data, _stamp, state):
    ...
    if measurement["active"]:
        ...
        _add_tone(
            signal,
            samples,
            frequency_hz=pinger_frequency_hz,
            sample_rate_hz=cfg.sample_rate_hz,
            sound_speed_mps=cfg.sound_speed_mps,
            amplitude=amplitude,
            phase_rad=pinger_phase_noise_rad,
...
def _update_pinger_phase_noise(
    bridge: Any,
    cfg: HydrophoneConfig,
    frames: int,
) -> float:
    """Advance a continuous, low-frequency pinger phase-wander state.

    The July 7 stationary bag's quiet opening had about 0.19 rad RMS residual
    phase after a ten-second linear fit.  An Ornstein-Uhlenbeck state models
    that bounded oscillator/path wander without pretending it is vehicle
    Doppler.  A zero standard deviation preserves the legacy deterministic
    carrier exactly.
    """

    sigma = float(cfg.pinger_phase_noise_std_rad)
    if sigma <= 0.0:
        bridge._hydrophone_pinger_phase_noise_rad = 0.0
        return 0.0
    elapsed_s = max(float(frames) / float(cfg.sample_rate_hz), 1.0e-9)
    correlation_s = max(float(cfg.pinger_phase_noise_correlation_s), 1.0e-6)
    rho = math.exp(-elapsed_s / correlation_s)
    innovation_std = sigma * math.sqrt(max(1.0 - rho * rho, 0.0))
    previous = float(getattr(bridge, "_hydrophone_pinger_phase_noise_rad", 0.0))
    current = rho * previous + innovation_std * float(bridge._hydrophone_rng.normal())
    bridge._hydrophone_pinger_phase_noise_rad = current
    return current


def _normalized_thruster_activity(bridge: Any, data, cfg: HydrophoneConfig) -> float:
    """Return an RMS actuator activity normalized to the rosbag calibration."""

    normalized_controls: list[float] = []
    for _site_name, actuator_name in THRUSTER_NOISE_SITES:
        actuator_id = _lookup_actuator(bridge.model, actuator_name)
        if actuator_id < 0:
            continue
        try:
            control = float(data.ctrl[actuator_id])
            limits = np.asarray(bridge.model.actuator_ctrlrange[actuator_id], dtype=np.float64)
            denominator = float(max(abs(limits[0]), abs(limits[1]), 1.0e-9))
        except Exception:
            continue
        normalized_controls.append(float(np.clip(abs(control) / denominator, 0.0, 1.0)))
    if not normalized_controls:
        return 0.0
    raw_rms = float(np.sqrt(np.mean(np.square(normalized_controls))))
    return float(np.clip(raw_rms / max(cfg.rosbag_thruster_activity_reference, 1.0e-6), 0.0, 1.0))


def _add_rosbag_20260707_stationary_tone(
    signal: np.ndarray,
    samples: np.ndarray,
    *,
    cfg: HydrophoneConfig,
    ...
    """Add the bag's stationary, channel-common 21.333 kHz nuisance line.

    With the pinger out of range, this 0.021 peak-amplitude line plus the
    configured 0.005 receiver floor yields about 0.0157 full-scale RMS, which
    matches the quiet opening of the July 7 recording.  It is deliberately
    stronger than a distant target and close to 21,164 Hz so frequency
    selection is tested against the observed false-lock condition.
    """

    if cfg.noise_profile != "rosbag_20260707" or cfg.rosbag_stationary_tone_amplitude <= 0.0:
        return
    _add_tone(
        signal,
        samples,
        frequency_hz=cfg.rosbag_stationary_tone_frequency_hz,
        sample_rate_hz=cfg.sample_rate_hz,
        sound_speed_mps=cfg.sound_speed_mps,
        amplitude=cfg.rosbag_stationary_tone_amplitude,
        phase_rad=0.0,
...
def _rosbag_colored_noise(
    rng: np.random.Generator,
    frames: int,
    sample_rate_hz: int,
) -> np.ndarray:
    """Synthesize the measured 23.8--24.4 kHz propulsion-noise envelope."""

    white = rng.normal(0.0, 1.0, size=frames)
    spectrum = np.fft.rfft(white)
    frequencies = np.fft.rfftfreq(frames, 1.0 / float(sample_rate_hz))
    envelope = (
        0.26
        + 1.35 * np.exp(-0.5 * np.square((frequencies - 23890.0) / 720.0))
        + 0.95 * np.exp(-0.5 * np.square((frequencies - 24395.0) / 260.0))
    )
    colored = np.fft.irfft(spectrum * envelope, n=frames)
    rms = float(np.sqrt(np.mean(np.square(colored))))
    if rms <= 1.0e-12:
        return np.zeros(frames, dtype=np.float32)
    return np.asarray(colored / rms, dtype=np.float32)


def _add_rosbag_20260707_thruster_noise(
    bridge: Any,
    data,
    signal: np.ndarray,
    samples: np.ndarray,
    *,
    ...
    """Add the July 7 arm/thruster-correlated broadband and clipping stress.

    Calibration targets from the real bag's armed interval are approximately
    0.411 full-scale RMS and 6.9 percent saturated samples.  The signal uses a
    colored continuous component plus heavy-tailed cavitation/electrical
    impulses.  It is gated by measured MuJoCo actuator activity, never by an
    oracle arm flag, so movement and ALT_HOLD corrections naturally drive it.
    """

    del samples, sample_offset
    cfg = bridge._hydrophone_config
    if cfg.noise_profile != "rosbag_20260707" or signal.size == 0:
        return
    activity = _normalized_thruster_activity(bridge, data, cfg)
    bridge._hydrophone_last_thruster_activity = activity
    if activity <= 0.0:
        return

    frames = signal.shape[0]
    common = _rosbag_colored_noise(bridge._hydrophone_rng, frames, cfg.sample_rate_hz)
    common *= np.float32(cfg.rosbag_thruster_broadband_rms * activity)
    probability = float(np.clip(cfg.rosbag_thruster_impulse_probability * activity, 0.0, 1.0))
    impulse_mask = bridge._hydrophone_rng.random(frames) < probability
    if np.any(impulse_mask):
        signs = bridge._hydrophone_rng.choice(np.array([-1.0, 1.0], dtype=np.float32), size=int(np.count_nonzero(impulse_mask)))
        common[impulse_mask] += (
            np.float32(cfg.rosbag_thruster_impulse_amplitude * activity) * signs
        )

    common_fraction = float(np.clip(cfg.rosbag_thruster_common_fraction, 0.0, 1.0))
    independent_scale = math.sqrt(max(1.0 - common_fraction * common_fraction, 0.0))
    for channel in range(signal.shape[1]):
        signal[:, channel] += np.float32(common_fraction) * common
        if independent_scale > 0.0:
            independent = _rosbag_colored_noise(
                bridge._hydrophone_rng, frames, cfg.sample_rate_hz
            )
            signal[:, channel] += np.float32(
                independent_scale * cfg.rosbag_thruster_broadband_rms * activity
            ) * independent
...
def schedule_hydrophone_jobs(bridge, jobs, add_rate_limited, *, builders: dict[str, object]) -> None:
    cfg = bridge._hydrophone_config
    if not cfg.enabled:
        return
    sync_hz = float(
