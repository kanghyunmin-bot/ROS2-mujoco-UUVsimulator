...
def build_sensor_frame_specs(
    *,
    ...
    imu_pos, imu_quat = site_local_pose(model, imu_site_id)
    bar30_pos, bar30_quat = site_local_pose(
        model,
        bar30_site_id,
        fallback_pos=np.array([-0.1736, -0.0303, 0.0536], dtype=np.float64),
    )
    dvl_pos, _dvl_site_quat = site_local_pose(
        model,
        dvl_site_id,
        fallback_pos=np.array([-0.0320, 0.0, -0.0970], dtype=np.float64),
        fallback_quat=x180,
    )
    ...
    return [
        ...
        ("base_link", "dvl", dvl_pos, ident),
        ("dvl", "dvl_link", zero, ident),
