...
def root_launchers() -> list[Path]:
    return [
        REPO_ROOT / "sim" / "run_mujoco.sh",
        REPO_ROOT / "sim" / "start_sitl_mujoco.sh",
        REPO_ROOT / "sim" / "reset_sim.sh",
    ]
...
def runtime_dirty_paths() -> str:
    return git_output(
        [
            ...
            "README.md",
            "setup",
            "sim/current/gui/uuv_control_gui.py",
        ],
        REPO_ROOT,
    )
...
def alias_text() -> str:
    ...
    try:
        ...
        return f"sim/current -> {target}"
    except OSError:
        return f"sim/current -> {ACTIVE_RUNTIME_ALIAS.resolve()}"
