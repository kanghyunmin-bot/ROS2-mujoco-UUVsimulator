...
def enter_requested_mode_and_release_hold(node, args: argparse.Namespace) -> None:
    ...
    # A reported mode change can precede EKF3's active/inertial-navigation
    # hand-off by several seconds.  Keep publishing neutral RC while the
    # simulator still owns the initial depth so that estimator transients do
    # not become real vehicle motion at release.
    node.spin_with_rc(
        max(0.0, float(args.post_mode_settle_s)),
        hz=args.publish_hz,
        input_mode=args.input_mode,
    )
