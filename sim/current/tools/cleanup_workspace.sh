...
DOC_DIR="${ROOT_DIR}/analysis"
KEEP_RUN="${KEEP_RUN:-closed_loop_joy_frame_fixed_60_180_20260501_192053}"
UUV_RUNTIME_DIR="${UUV_MUJOCO_RUNTIME_DIR:-${ROOT_DIR}/sim/current}"
if [[ ! -d "${UUV_RUNTIME_DIR}" ]]; then
  echo "[cleanup] active runtime not found: ${UUV_RUNTIME_DIR}" >&2
  echo "[cleanup] Expected sim/current, or set UUV_MUJOCO_RUNTIME_DIR explicitly." >&2
  exit 1
fi
