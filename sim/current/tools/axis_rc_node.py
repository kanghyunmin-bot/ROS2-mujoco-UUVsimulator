...
class AxisRcOverrideCheck(AxisRcNodeCallbacks, AxisRcNodeControl, AxisRcNodeServices, Node):
    def __init__(
        ...
        # These are sensor-data publishers in both the real MAVROS graph and
        # the simulator.  A reliable subscription is incompatible with their
        # best-effort QoS and previously made the acceptance log silently fill
        # IMU/odometry fields with NaN.
        self.create_subscription(Imu, "/mavros/imu/data", self._on_imu, qos_profile_sensor_data)
        self.create_subscription(TwistStamped, "/dvl/velocity", self._on_dvl_twist, qos_profile_sensor_data)
        self.create_subscription(Float32, "/depth", self._on_depth, qos_profile_sensor_data)
