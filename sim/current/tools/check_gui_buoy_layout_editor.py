...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="uuv_buoy_layout_") as tmp:
        ...
        rod_name = f"{fixed.prefix}_mooring_rod"
        before_rod_endpoints = floats(before_geoms[rod_name].get("fromto"))
        ...
        after_rod_endpoints = floats(geoms[rod_name].get("fromto"))
        if after_rod_endpoints != before_rod_endpoints:
            raise AssertionError("fixed mooring rod local geometry changed during layout save")
        if any("_flex_line_" in (element.get("name") or element.get("prefix") or "") for element in root.iter()):
            raise AssertionError("legacy flexible cable elements returned during layout save")
        for geom_name in fixed.projection_geom_names:
