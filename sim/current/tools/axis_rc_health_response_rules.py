...
def check_neutral_residuals(
    ...
    *,
    ...
    depth_span = float(row.get("depth_m_span", float("nan")))
    if math.isfinite(depth_span) and depth_span > 0.15:
        severity = apply_failure_flag(flags, severity, "neutral_depth_hold_lost")
    elif math.isfinite(depth_span) and depth_span > 0.08:
        severity = apply_warning_flag(flags, severity, "neutral_depth_hold_drift")
    return severity
