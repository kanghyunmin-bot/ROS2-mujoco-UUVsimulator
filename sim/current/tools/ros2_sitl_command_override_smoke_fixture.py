...
class FakeBridge:
    def __init__(self) -> None:
        ...
        self.direct_calls: list[tuple[float, float, float, float]] = []
    ...
    def _parse_command_override_payload(self, text: str):
        return {
            ...
            "direct_cmd": {"forward": 0.7, "sway": -0.2, "heave": 0.1, "yaw": -0.3},
            "arm": "true",
            "mode": text,
        }
    ...
    def _handle_normalized_cmd(self, fwd: float, sway: float, yaw: float, heave: float) -> None:
        self.direct_calls.append((float(fwd), float(sway), float(yaw), float(heave)))
