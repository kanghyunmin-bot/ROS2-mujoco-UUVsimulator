...
def build_rc_input_checks(paths: dict[str, Path]) -> list[Check]:
    ...
    legacy_axis_mapping = contains_all(
        joystick_cpp,
        [
            "set_override(2",
            "set_override(3",
    ...
    channel_axis_mapping = contains_all(
        joystick_cpp,
        [
            "channel_throttle->set_override", "channel_yaw->set_override",
            "channel_forward->set_override", "channel_lateral->set_override",
        ],
    )
    ok = legacy_axis_mapping or channel_axis_mapping
    checks.append(
        Check(
            ...
            evidence=[
                evidence(joystick_cpp, "channel_throttle->set_override", "RC3 throttle/heave"),
                evidence(joystick_cpp, "channel_yaw->set_override", "RC4 yaw"),
                evidence(joystick_cpp, "channel_forward->set_override", "RC5 forward"),
                evidence(joystick_cpp, "channel_lateral->set_override", "RC6 lateral"),
            ],
            official_refs=[],
        )
    )
