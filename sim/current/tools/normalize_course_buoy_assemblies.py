...
ASSEMBLY_TARGET_HEIGHT_M = 0.400
ASSEMBLY_TOP_Z_M = 0.194
MOVING_STICK_TOP_Z_M = -0.063
MOVING_STICK_ORIGINAL_BOTTOM_Z_M = -0.170
MOVING_STICK_ORIGINAL_LENGTH_M = (
    MOVING_STICK_TOP_Z_M - MOVING_STICK_ORIGINAL_BOTTOM_Z_M
)
MOVING_ATTACH_SITE_Z_M = ASSEMBLY_TOP_Z_M - ASSEMBLY_TARGET_HEIGHT_M
MOVING_STICK_LENGTH_M = MOVING_STICK_TOP_Z_M - (MOVING_ATTACH_SITE_Z_M + 0.045)
MOVING_STICK_EXTENSION_M = MOVING_STICK_LENGTH_M - MOVING_STICK_ORIGINAL_LENGTH_M
MOVING_STICK_BOTTOM_Z_M = MOVING_STICK_TOP_Z_M - MOVING_STICK_LENGTH_M
MOVING_LOWER_JIG_Z_M = -0.183 - MOVING_STICK_EXTENSION_M
MOVING_MAGNET_PLATE_Z_M = -0.195 - MOVING_STICK_EXTENSION_M
MOVING_MAGNET_Z_M = -0.205 - MOVING_STICK_EXTENSION_M
MOORING_ROD_BOTTOM_Z_M = 0.109
MOORING_ROD_TOP_Z_M = 2.195 - MOVING_STICK_EXTENSION_M
MOORING_TOP_JIG_Z_M = 2.205 - MOVING_STICK_EXTENSION_M
MOORING_FIXED_MAGNET_STEM_BOTTOM_Z_M = 2.205 - MOVING_STICK_EXTENSION_M
MOORING_FIXED_MAGNET_STEM_TOP_Z_M = 2.230 - MOVING_STICK_EXTENSION_M
MOORING_FIXED_MAGNET_Z_M = 2.240 - MOVING_STICK_EXTENSION_M
MOORING_MAGNET_SITE_Z_M = 2.250 - MOVING_STICK_EXTENSION_M
    ...
    while index < len(source):
        ...
        output.extend(
            (
                ...
                f'fromto="0 0 {MOORING_ROD_BOTTOM_Z_M:.4f} 0 0 {MOORING_ROD_TOP_Z_M:.4f}" '
                ...
                f'pos="0 0 {MOORING_TOP_JIG_Z_M:.4f}" size="0.028 0.010" '
                'rgba="0.95 0.95 0.92 1" contype="0" conaffinity="0" group="0" />',
                f'{child}<geom name="{prefix}_fixed_magnet_stem" type="capsule" '
                f'fromto="0 0 {MOORING_FIXED_MAGNET_STEM_BOTTOM_Z_M:.4f} '
                f'0 0 {MOORING_FIXED_MAGNET_STEM_TOP_Z_M:.4f}" size="0.0075" '
                'rgba="0.92 0.92 0.88 1" contype="0" conaffinity="0" group="0" />',
                f'{child}<geom name="{prefix}_fixed_magnet" type="cylinder" '
                f'pos="0 0 {MOORING_FIXED_MAGNET_Z_M:.4f}" size="0.018 0.010" '
                'rgba="0.04 0.04 0.04 1" '
                'contype="0" conaffinity="0" group="0" />',
                f'{child}<site name="{prefix}_magnet_site" pos="0 0 {MOORING_MAGNET_SITE_Z_M:.4f}" '
...
def normalize_moving_sticks(source: list[str], moored_prefixes: set[str]) -> list[str]:
    """Size the moving stick so the complete detachable assembly is 400 mm."""

    output: list[str] = []
    for line in source:
        if "_pvc_contrast_band_" in line:
            continue
        updated = line
        for prefix in moored_prefixes:
            if f'name="{prefix}_pvc_pipe"' in updated:
                updated = replace_vector(
                    updated,
                    FROMTO_RE,
                    lambda _parts: [
                        0.0,
                        0.0,
                        MOVING_STICK_TOP_Z_M,
                        0.0,
                        0.0,
                        MOVING_STICK_BOTTOM_Z_M,
                    ],
                )
                updated = re.sub(
                    r'\s+(?:rgba|specular|shininess|material)="[^"]+"',
                    '',
                    updated,
                )
                updated = updated.replace(
                    ' density="0"',
                    ' material="course_buoy_pvc" density="0"',
                    1,
                )
                break
            if (
                f'name="{prefix}_moving_lower_jig"' in updated
                or f'name="{prefix}_pinger_bottom_jig"' in updated
            ):
                updated = replace_vector(
                    updated,
                    POSITION_RE,
                    lambda parts: [parts[0], parts[1], MOVING_LOWER_JIG_Z_M],
                )
                break
            if f'name="{prefix}_magnet_lower_plate"' in updated:
                updated = replace_vector(
                    updated,
                    POSITION_RE,
                    lambda parts: [parts[0], parts[1], MOVING_MAGNET_PLATE_Z_M],
                )
                break
            if f'name="{prefix}_magnet"' in updated:
                updated = replace_vector(
                    updated,
                    POSITION_RE,
                    lambda parts: [parts[0], parts[1], MOVING_MAGNET_Z_M],
                )
                break
            if f'name="{prefix}_attach_site"' in updated:
                updated = replace_vector(
                    updated,
                    POSITION_RE,
                    lambda parts: [parts[0], parts[1], MOVING_ATTACH_SITE_Z_M],
                )
                break
...
def normalize_stick_material(source: list[str]) -> list[str]:
    """Install one uniform PVC material that gains contrast from scene lighting."""

    output = [
        line for line in source
        if '<material name="course_buoy_pvc"' not in line
    ]
    for index, line in enumerate(output):
        if line.strip() == "<asset>":
            ...
            output.insert(
                index + 1,
                f'{indent}<material name="course_buoy_pvc" '
                'rgba="0.86 0.90 0.94 1" specular="0.35" shininess="0.45" />',
            )
            break
    return output
...
def normalize(path: Path) -> int:
    ...
    source = normalize_stick_material(source)
    source = normalize_moving_sticks(source, moored_prefixes)
