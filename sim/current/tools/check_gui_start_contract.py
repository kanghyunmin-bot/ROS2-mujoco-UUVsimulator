...
def check_native_gui_start_env() -> None:
    ...
    stable_ardupilot = workspace / "sim/ardupilot"
    expected_ardupilot = str(stable_ardupilot if stable_ardupilot.exists() else workspace / "sim" / "ardupilot")
...
def check_ardusub_rc_override_heave_trim() -> None:
    ...
    if "sitl_real_params_filtered_" not in text or "filtered sim-forced duplicate params" not in text:
        raise AssertionError("SITL startup must filter real-param keys that SITL forcibly overrides")
    if "FRAME_CONFIG|RCMAP_ROLL|RCMAP_PITCH|RCMAP_THROTTLE|RCMAP_YAW|RCMAP_FORWARD|RCMAP_LATERAL|RC1_DZ|RC2_DZ|RC3_DZ|RC3_MIN|RC3_MAX|RC3_TRIM|RC4_DZ|RC5_DZ|RC6_DZ|THR_DZ|JS_GAIN_DEFAULT|JS_GAIN_MIN|JS_GAIN_MAX|JS_GAIN_STEPS|JS_THR_GAIN" not in text:
        raise AssertionError("SITL pilot-input params must override the real-robot replay dump")
    ...
    if expected not in text:
        raise AssertionError("SITL arm path must not inherit real-vehicle ARMING_CHECK prearm blocks")
    expected = 'append_param_if_not_overridden "FS_GCS_ENABLE" "${SITL_FS_GCS_ENABLE:-0}"'
    if expected not in text:
        raise AssertionError("SITL arm path must disable real-vehicle GCS failsafe disarm")
    expected = 'append_param_if_not_overridden "FS_PILOT_INPUT" "${SITL_FS_PILOT_INPUT:-0}"'
    if expected not in text:
        raise AssertionError("SITL arm path must disable real-vehicle pilot-input failsafe disarm")
    expected = 'append_param_if_not_overridden "BRD_SAFETYENABLE" "${SITL_BRD_SAFETYENABLE:-0}"'
    if expected not in text:
        raise AssertionError("SITL arm path must force the safety switch off")
...
def check_reset_script_preserves_process_lineage() -> None:
    ...
    if "local_endpoint = endpoint.split(\"->\", 1)[0]" not in reset_text:
        raise AssertionError("reset UDP report must only show local bound ports")


def check_native_wrappers_default_to_stable_ardupilot() -> None:
    expected = "sim/ardupilot"
    for script_name in (
        "start_sitl_mujoco_mj311.sh",
        "start_ardusub_sitl_mj311.sh",
        "reset_uuv_sim.sh",
        ...
        if expected not in text:
            raise AssertionError(f"{script_name} must prefer sim/ardupilot for native SITL")
...
def main() -> int:
    ...
    check_reset_script_preserves_process_lineage()
