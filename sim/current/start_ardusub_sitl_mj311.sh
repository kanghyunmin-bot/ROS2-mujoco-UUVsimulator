...
resolve_default_workspace_dir() {
  ...
  while [[ -n "$dir" ]]; do
    current_score=0
    [[ -d "$dir/sim/ardupilot" ]] && current_score=$((current_score + 20))
...
if [[ -z "${ARDUPILOT_DIR:-}" && -d "${WORKSPACE_DIR}/sim/ardupilot" ]]; then
  ARDUPILOT_DIR="${WORKSPACE_DIR}/sim/ardupilot"
fi
...
is_sim_forced_param() {
  case "$1" in
    ...
    FRAME_CONFIG|RCMAP_ROLL|RCMAP_PITCH|RCMAP_THROTTLE|RCMAP_YAW|RCMAP_FORWARD|RCMAP_LATERAL|RC1_DZ|RC2_DZ|RC3_DZ|RC3_MIN|RC3_MAX|RC3_TRIM|RC4_DZ|RC5_DZ|RC6_DZ|THR_DZ|JS_GAIN_DEFAULT|JS_GAIN_MIN|JS_GAIN_MAX|JS_GAIN_STEPS|JS_THR_GAIN)
      return 0
      ;;
    # The physical attitude gains are unstable in the lower-inertia MuJoCo
    # plant after a horizontal-motion disturbance. Filter this bounded set so
    # the explicit SITL defaults/A-B overrides below actually win; the stored
    # hardware parameter contract remains untouched.
    ATC_ANG_RLL_P|ATC_ANG_PIT_P|ATC_ANG_YAW_P|ATC_RAT_RLL_P|ATC_RAT_RLL_I|ATC_RAT_RLL_D|ATC_RAT_PIT_P|ATC_RAT_PIT_I|ATC_RAT_PIT_D|ATC_RAT_YAW_P|ATC_RAT_YAW_I|ATC_RAT_YAW_D)
      return 0
      ;;
    # Preserve the hardware yaw-command mapping unless an explicit replay A/B
    # requests a different value. Without filtering here, the real parameter
    # dump silently masks SITL_ACRO_YAW_P in the later extra-param file.
    ACRO_YAW_P)
      if [[ -n "${SITL_ACRO_YAW_P:-}" ]]; then
        return 0
      fi
      return 1
      ;;
    ...
    PSC_ACCZ_I|PSC_ACCZ_IMAX|PSC_ACCZ_D|PSC_ACCZ_FLTD|PSC_ACCZ_FLTE|PSC_ACCZ_FLTT)
      ...
      return 1
      ;;
    # A/B support for the simulated pressure-reference contract.  The real
    # vehicle dump owns SURFACE_DEPTH by default; only filter it when an
    # explicit simulation test requests a different EKF-relative surface
    # ceiling.  Without this gate SITL_SURFACE_DEPTH appears accepted by the
    # launcher but is silently shadowed by the hardware parameter file.
    SURFACE_DEPTH)
      case "${SITL_FORCE_SURFACE_DEPTH:-0}" in
        1|true|TRUE|yes|YES|on|ON|enable|enabled)
          return 0
          ;;
      esac
...
case "${SITL_SERIAL0_UDPCLIENT:-0}" in
  1|true|TRUE|yes|YES|on|ON|enable|enabled)
    # Non-blocking bootstrap only for explicit direct-MAVLink experiments.
...
# Direct SITL can expose an explicit serial1 QGC link for A/B debugging.
...
# a brief host scheduling stall during JSON startup causes watchdog_rst,
...
# Start from the real-vehicle parameter contract, then apply the bounded
# simulation-only attitude gains below. The checkout is pinned to
# ArduSub-4.1.2, so these names intentionally match the older QGC names.
append_param_if_not_overridden "ATC_INPUT_TC" "0.15"
append_param_if_not_overridden "ATC_ANG_RLL_P" "${SITL_ATC_ANG_RLL_P:-4.5}"
append_param_if_not_overridden "ATC_ANG_PIT_P" "${SITL_ATC_ANG_PIT_P:-4.5}"
append_param_if_not_overridden "ATC_ANG_YAW_P" "${SITL_ATC_ANG_YAW_P:-2.0}"
# Keep hardware parity by default, while allowing an explicit replay-only
# yaw-command calibration without changing the stored real-vehicle dump.
append_param_if_not_overridden "ACRO_YAW_P" "${SITL_ACRO_YAW_P:-3.375}"
...
append_param_if_not_overridden "ATC_RAT_RLL_P" "${SITL_ATC_RAT_RLL_P:-0.08}"
append_param_if_not_overridden "ATC_RAT_RLL_I" "${SITL_ATC_RAT_RLL_I:-0.08}"
append_param_if_not_overridden "ATC_RAT_RLL_IMAX" "0.02"
# The hardware dump uses 0.020 and excites a roll-rate limit cycle in the
# lower-inertia MuJoCo plant after a movement disturbance. Keep the hardware
# file untouched and use a simulation-only stable default;
# SITL_ATC_RAT_RLL_D can still request hardware parity for an explicit replay.
append_param_if_not_overridden "ATC_RAT_RLL_D" "${SITL_ATC_RAT_RLL_D:-0.0015}"
...
append_param_if_not_overridden "ATC_RAT_PIT_P" "${SITL_ATC_RAT_PIT_P:-0.08}"
append_param_if_not_overridden "ATC_RAT_PIT_I" "${SITL_ATC_RAT_PIT_I:-0.03}"
append_param_if_not_overridden "ATC_RAT_PIT_IMAX" "0.0"
# Leaving only pitch at the physical 0.020 value drives SERVO5..8 into an
# alternating (+,-,+,-) limit cycle in neutral ALT_HOLD. Preserve the hardware
# dump and use the same simulation-only derivative default as roll.
append_param_if_not_overridden "ATC_RAT_PIT_D" "${SITL_ATC_RAT_PIT_D:-0.0015}"
...
append_param_if_not_overridden "ATC_RAT_YAW_P" "${SITL_ATC_RAT_YAW_P:-0.12}"
append_param_if_not_overridden "ATC_RAT_YAW_I" "${SITL_ATC_RAT_YAW_I:-0.0}"
append_param_if_not_overridden "ATC_RAT_YAW_IMAX" "0.02"
append_param_if_not_overridden "ATC_RAT_YAW_D" "${SITL_ATC_RAT_YAW_D:-0.0036}"
