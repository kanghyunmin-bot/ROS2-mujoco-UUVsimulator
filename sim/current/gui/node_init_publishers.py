...
def initialize_command_publishers_and_clients(self) -> None:
    ...
    # /mavros/rc/override publisher while the autonomous controller owns the
...
def _suspend_rc_override_service(self, request, response):
    ...
    response.message = (
        "web GUI RC publisher suspended; pinger controller owns /mavros/rc/override directly"
        if response.success
...
def suspend_rc_override_publisher(self) -> bool:
    ...
    ``/mavros/rc/override`` while its exclusive controller is running. Publishing
