        ...
        def wait_for_vehicle() -> None:
            ...
            while time.monotonic() < deadline and not self._stop.is_set():
                ...
                if (
                    ...
                    # ALT_HOLD must capture its target only after EKF3 has
                    # converged to the current Bar30 depth.  Entering it on the
                    # first heartbeat captures Z=0 (the water surface), then
                    # the vehicle drives upward as soon as control is enabled.
                    # Keep the neutral plant in MANUAL while arm/EKF readiness
                    # settles, then switch to ALT_HOLD last.
                    self.enqueue_mode_command("MANUAL", label="pinger_homing_manual")
                    self.node.push_event("web pinger homing requested MANUAL for EKF settle")

                    manual_deadline = time.monotonic() + 15.0
                    while time.monotonic() < manual_deadline and not self._stop.is_set():
                        if not activation_current() or not self.processes.pinger_homing_running():
                            return
                        if str(self.node.snapshot().mode).upper() == "MANUAL":
                            break
                        time.sleep(0.10)

                    # A first MAVROS state can arrive while ArduSub is still
                    # completing its startup checks. Retry an owned auto-arm,
                    # or wait for an operator/external arm, before starting the
                    # estimator settle interval.
                    arm_deadline = time.monotonic() + (20.0 if auto_arm else 120.0)
                    attempt = 0
                    ...
                    ekf_settle_s = max(
                        0.0,
                        float(os.environ.get("UUV_GUI_PINGER_EKF_SETTLE_S", "15.0")),
                    )
                    self.node.push_event(
                        f"web pinger homing holding MANUAL neutral for EKF ({ekf_settle_s:.1f}s)"
                    )
                    settle_deadline = time.monotonic() + ekf_settle_s
                    while time.monotonic() < settle_deadline and not self._stop.is_set():
                        if not activation_current() or not self.processes.pinger_homing_running():
                            return
                        ready = self.node.snapshot()
                        if not bool(ready.connected) or not bool(ready.armed):
                            self.node.push_event("web pinger homing EKF settle aborted: vehicle not armed")
                            return
                        time.sleep(0.10)

                    self.enqueue_mode_command(mode, label="pinger_homing_mode")
                    self.node.push_event(f"web pinger homing mode requested after EKF settle: {mode}")
                    mode_deadline = time.monotonic() + 20.0
                    while time.monotonic() < mode_deadline and not self._stop.is_set():
                        if not activation_current() or not self.processes.pinger_homing_running():
                            return
                        if str(self.node.snapshot().mode).upper() == str(mode).upper():
                            self.node.push_event(
                                "web pinger homing ALT_HOLD confirmed; acoustic controller may own RC"
                            )
                            return
                        time.sleep(0.10)
                    if activation_current() and not self._stop.is_set():
                        self.node.push_event("web pinger homing ALT_HOLD confirmation timeout")
                    return
                time.sleep(0.10)
            if activation_current() and not self._stop.is_set():
