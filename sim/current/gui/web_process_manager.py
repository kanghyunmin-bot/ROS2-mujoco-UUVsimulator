...
class WebProcessManager:
    ...
    def start_sim_stack(self, *, purpose: str | None = None) -> dict[str, Any]:
        ...
        # Keep the pinger-purpose acoustic profile selected above.  It uses
        # the July 7 bag-calibrated actuator noise; these bounds still place
        # any explicitly requested spatial interferers inside the test tank.
    ...
    def start_pinger_homing(self, values: dict[str, Any]) -> dict[str, Any]:
        ...
        homing_selection = str(
            values.get("algorithm", "no_odom_phase") or "no_odom_phase"
        ).strip().lower()
        if homing_selection != "no_odom_phase":
            self._pinger_homing_status = (
                "pinger homing: simulator parity mode supports the physical no-odometry Phase ABBA "
                "contract only"
            )
            return {"status": self._pinger_homing_status, "running": False}
        requested_navigation_mode = "no_odom_phase"
        homing_algorithm = "phase"
        reference_frequency_hz = _float_range_or_default(
            values.get("reference_frequency_hz", 21164.0),
            default=21164.0,
            lower=1000.0,
            upper=47000.0,
        )
        ...
        probe_pwm_delta = _int_range(
            values.get("no_odom_probe_pwm_delta", 90),
            lower=1,
            upper=250,
        )
        approach_pwm_delta = _int_range(
            values.get("no_odom_approach_pwm_delta", 200),
            lower=1,
            upper=250,
        )
        if test_tank_active:
            # A full 200-PWM competition surge can cross half of this 5.49 m
            # tank before the next acoustic innovation window.  Preserve a
            # decisive moving approach while leaving room for feedback turns.
            approach_pwm_delta = min(approach_pwm_delta, 120)
        approach_duration_s = _float_range(
            values.get("no_odom_forward_duration_s", 40.0),
            lower=1.0,
            upper=180.0,
        )
        ...
        # One full ABBA probe already contains two excitations in each sign
        # for both horizontal axes.  Requiring two *whole* ABBA probes while
        # carrying the competition forward bias traverses most of the 5.49 m
        ...
        launch_arguments = [
            ...
            # On a loaded MuJoCo viewer the SITL heartbeat follows simulation
            # time (RTF can fall below 0.4), while this real-package launch
            # evaluates receive freshness on steady time. Keep the physical
            # 3.5 s default and relax only the simulator wrapper.
            "vehicle_state_timeout_s:=15.0",
            ...
            f"approach_duration_s:={approach_duration_s:.6f}",
            f"probe_leg_s:={probe_leg_s:.6f}",
            f"probe_neutral_s:={probe_neutral_s:.6f}",
            ...
            # Keep the geometric fallback conservative as a separate guard;
            # calibrated acoustic range is the authoritative simulator stop.
            "arrival_radius_m:=0.8",
            f"max_runtime_s:={max_runtime_s:.6f}",
            # End-to-end calibration of the synthetic PCM -> phase-estimator
            # chain gives |IQ|*sqrt(range)=0.01950 after setting a competition
            # pinger source level that clears the physical estimator's 2.0
            # IQ-SNR gate at the 22 m start. Runtime range therefore uses
            # (0.01950/|IQ|)^2 and consumes received audio only;
        ...
        mode_label = f"NO_ODOM_PHASE_ABBA_FIXED_{reference_frequency_hz:.0f}HZ"
        self.node.push_event(
            f"pinger homing mode: {mode_label} "
            f"(estimator={homing_algorithm}, odometry=not-required)"
        )
        self.node.push_event(
            "pinger homing tuning: "
            f"reference={reference_frequency_hz:.3f}Hz (fixed; FFT selection disabled), "
            f"probe_pwm_delta={probe_pwm_delta}, "
            f"approach_pwm_delta={approach_pwm_delta}, "
            f"ABBA={initial_confirmation_probes}x/{probe_leg_s:.2f}s, "
            f"forward_bias={probe_forward_bias:.2f}, "
            f"acoustic_stop={success_range_m:.2f}m, "
        ...
        result["tuning"] = {
            ...
            "initial_confirmation_probes": initial_confirmation_probes,
            "probe_leg_s": probe_leg_s,
            "probe_forward_bias": probe_forward_bias,
    ...
    def start_vision_processing(self) -> dict[str, Any]:
        ...
        model_path = APP_ROOT / "sim" / "current" / "assets" / "yolo" / "best.pt"
        command_parts = [
            "env",
            "OMP_NUM_THREADS=1",
            "MKL_NUM_THREADS=1",
    ...
    def start_ground_truth_mission(self, values: dict[str, Any]) -> dict[str, Any]:
        ...
        if observation_launch.is_file():
            ...
            vision_model_path = APP_ROOT / "sim" / "current" / "assets" / "yolo" / "best.pt"
        ...
        if cpp_runner.is_file() and os.access(cpp_runner, os.X_OK):
            ...
            self._mission_status = f"mission missing: {cpp_runner}"
    ...
    def _watch_process(self, *, proc: Any, log_path: Path, label: str, attr_name: str, status_attr: str) -> None:
        ...
        if label == "pinger homing":
            matcher = IncrementalLogMatcher(
                log_path,
                ("-> COMPLETE", "-> FAILED_TIMEOUT", "-> FAILED_ESTIMATE", "-> FAILED_DEPTH"),
            )
...
def _pinger_homing_terminal_log_status(path: Path) -> str:
    ...
    if "-> FAILED_DEPTH" in log_text:
        return "pinger homing failed: maximum vehicle depth reached"
    return ""
