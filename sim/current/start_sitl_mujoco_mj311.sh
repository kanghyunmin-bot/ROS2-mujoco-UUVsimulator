...
if [[ "$ROS2_MODE" == "compat" ]]; then
  if [[ "$SITL_DIRECT_MAVLINK" -eq 1 ]]; then
    echo "[start] error: --ros2-real-pkg-compat cannot be combined with --direct-mavlink." >&2
    echo "[start]        Strict compatibility requires legacy MAVProxy fan-out to external MAVROS." >&2
    exit 2
  fi
  export SITL_MAVROS_OUTPUT_ENABLE=1
  export SITL_MAVROS_HOST="${SITL_MAVROS_HOST:-127.0.0.1}"
  export SITL_MAVROS_PORT="${SITL_MAVROS_PORT:-14551}"
  # Keep the real A50/EKF surface above 10 Hz even when MuJoCo runs below
  # wall-clock real time under camera and MAVROS load.
  export ROS2_UUV_DVL_TWIST_HZ="${ROS2_UUV_DVL_TWIST_HZ:-60}"
  export SITL_MAV_GCS_SYSID="${SITL_MAV_GCS_SYSID:-1}"
  export SITL_MAV_GCS_SYSID_HI="${SITL_MAV_GCS_SYSID_HI:-255}"
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DEFAULT_REAL_START_STATE_CSV="$(cd "${SCRIPT_DIR}/../.." && pwd)/analysis/controller_debug/controller_parity_412/real_20260401_feedback/real_controller_feedback_20hz.csv"
...
if [[ -z "${ARDUPILOT_DIR:-}" && -d "${WORKSPACE_DIR}/sim/ardupilot" ]]; then
  export ARDUPILOT_DIR="${WORKSPACE_DIR}/sim/ardupilot"
fi
...
ACTIVE_RUNTIME_DIR="${UUV_MUJOCO_RUNTIME_DIR:-${WORKSPACE_DIR}/sim/current}"
...
SITL_INSTANCE_REQUESTED="${SITL_INSTANCE:-auto}"
if [[ "$SITL_INSTANCE_REQUESTED" == "auto" ]]; then
  ...
  if [[ -z "$SITL_INSTANCE_EFFECTIVE" ]]; then
    echo "[start] no free ArduPilot SITL TCP port in 5760..5850" >&2
    exit 1
  fi
elif [[ "$SITL_INSTANCE_REQUESTED" =~ ^[0-9]+$ ]]; then
  SITL_INSTANCE_EFFECTIVE="$SITL_INSTANCE_REQUESTED"
else
  echo "[start] SITL_INSTANCE must be auto or a non-negative integer: $SITL_INSTANCE_REQUESTED" >&2
  exit 1
fi
export SITL_INSTANCE="$SITL_INSTANCE_EFFECTIVE"
SITL_TCP_MAVLINK_PORT_EFFECTIVE=$((5760 + 10 * SITL_INSTANCE_EFFECTIVE))
SITL_JSON_SERVO_PORT_BASE="${SITL_JSON_SERVO_PORT:-9002}"
SITL_JSON_SENSOR_PORT_BASE="${SITL_JSON_SENSOR_PORT:-9003}"
SITL_JSON_SERVO_PORT_EFFECTIVE=$((SITL_JSON_SERVO_PORT_BASE + 10 * SITL_INSTANCE_EFFECTIVE))
SITL_JSON_SENSOR_PORT_EFFECTIVE=$((SITL_JSON_SENSOR_PORT_BASE + 10 * SITL_INSTANCE_EFFECTIVE))

echo "[start] step 2/3: start ArduSub SITL"
echo "[start] SITL instance=${SITL_INSTANCE_EFFECTIVE} TCP master=${SITL_TCP_MAVLINK_PORT_EFFECTIVE}"
echo "[start] transport: JSON servo=udp://127.0.0.1:${SITL_JSON_SERVO_PORT_EFFECTIVE} sensor=udp://127.0.0.1:${SITL_JSON_SENSOR_PORT_EFFECTIVE}"
if [[ "$ROS2_MODE" == "compat" ]]; then
  echo "[start] compatibility mode: strict external real-package parity"
  echo "[start] external MAVROS endpoint: udp://${SITL_MAVROS_HOST}:${SITL_MAVROS_PORT}"
  echo "[start] command path: /mavros/rc/override -> external mavros_node -> ArduSub -> UDP 9002 -> MuJoCo"
else
  echo "[start] compatibility mode: lightweight simulator ROS/MAVROS surface (${ROS2_MODE})"
fi
...
for ((i=1; i<=WAIT_SECS; i++)); do
  ...
  if [[ "$SITL_PARAM_TUNE" -eq 1 || "$SITL_DIRECT_MAVLINK" -eq 0 ]]; then
    if log_has_text "Connect tcp:127.0.0.1:${SITL_TCP_MAVLINK_PORT_EFFECTIVE} source_system=255" \
      && ardusub_log_has_text "Connection on serial port ${SITL_TCP_MAVLINK_PORT_EFFECTIVE}"; then
      SITL_READY=1
...
LAUNCH_ARGS=(
  --sitl
  --sitl-port "${SITL_JSON_SERVO_PORT_EFFECTIVE}"
  --sitl-send-port "${SITL_JSON_SENSOR_PORT_EFFECTIVE}"
...
if ((${#MUJOCO_EXTRA_ARGS[@]} > 0)); then
  ...
  "$LAUNCH_SCRIPT" "${LAUNCH_ARGS[@]}" >"$LAUNCH_LOG" 2>&1 &
	...
	if [[ "$ROS2_MODE" == "compat" ]]; then
	  HEARTBEAT_CHECKER="${SCRIPT_DIR}/tools/wait_mavlink_heartbeat.py"
	  # MAVProxy may finish the initial 1367-parameter download just after one
	  # minute on the full competition scene.  Keep strict heartbeat readiness,
	  # but allow that normal cold-start path to complete.
	  HEARTBEAT_TIMEOUT_S="${SITL_EXTERNAL_MAVROS_HEARTBEAT_TIMEOUT_S:-90}"
	  echo "[start] verifying fresh ArduSub heartbeat on external MAVROS endpoint ${SITL_MAVROS_HOST}:${SITL_MAVROS_PORT}"
	  if ! "${MJ311_PYTHON:-python3}" "$HEARTBEAT_CHECKER" \
	      --bind-host "${SITL_MAVROS_BIND_HOST:-0.0.0.0}" \
	      --port "$SITL_MAVROS_PORT" \
	      --timeout "$HEARTBEAT_TIMEOUT_S"; then
	    echo "[start] strict compatibility readiness failed: no fresh heartbeat on UDP ${SITL_MAVROS_PORT}" >&2
	    kill "$LAUNCH_PID" 2>/dev/null || true
	    exit 1
	  fi
	  echo "[start] strict compatibility transport READY"
	fi
