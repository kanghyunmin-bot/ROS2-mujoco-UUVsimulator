...
: config_(config)
{
  if (
    ...
    !finite(config_.circular_exclusion_center) ||
    !std::isfinite(config_.circular_exclusion_radius_m) ||
    !std::isfinite(config_.min_lane_segment_length_m))
  {
  ...
  if (
    ...
    config_.circular_exclusion_radius_m < 0.0 ||
    config_.min_lane_segment_length_m < 0.0)
  {
  ...
  const bool vertical = config_.lane_orientation == LaneOrientation::VERTICAL;
  const double coverage_span = vertical ? usable_length : usable_width;
  const double nominal_coverage_width = 2.0 * config_.lane_search_offset_m;
  const auto lane_count = static_cast<std::size_t>(
    std::max(1.0, std::ceil(coverage_span / nominal_coverage_width)));
  actual_lane_spacing_m_ = coverage_span / static_cast<double>(lane_count);
  lanes_.reserve(lane_count + 2U);
  ...
  for (std::size_t index = 0; index < lane_count; ++index) {
    if (!vertical) {
      ...
      append_lane({safe_bounds_.x_min, y}, {safe_bounds_.x_max, y});
      continue;
    }

    const double x = safe_bounds_.x_min +
      (static_cast<double>(index) + 0.5) * actual_lane_spacing_m_;
    const double radius = config_.circular_exclusion_radius_m;
    const double dx = x - config_.circular_exclusion_center.x;
    if (!config_.circular_exclusion_enabled || std::abs(dx) >= radius) {
      append_lane({x, safe_bounds_.y_min}, {x, safe_bounds_.y_max});
      continue;
    }

    const double dy = std::sqrt(std::max(0.0, radius * radius - dx * dx));
    const double exclusion_y_min = std::clamp(
      config_.circular_exclusion_center.y - dy,
      safe_bounds_.y_min, safe_bounds_.y_max);
    const double exclusion_y_max = std::clamp(
      config_.circular_exclusion_center.y + dy,
      safe_bounds_.y_min, safe_bounds_.y_max);
    const Vec2 lower_a{x, safe_bounds_.y_min};
    const Vec2 lower_b{x, std::min(safe_bounds_.y_max, exclusion_y_min)};
    const Vec2 upper_a{x, std::max(safe_bounds_.y_min, exclusion_y_max)};
    const Vec2 upper_b{x, safe_bounds_.y_max};
    if (distance(lower_a, lower_b) >= config_.min_lane_segment_length_m) {
      append_lane(lower_a, lower_b);
    }
    if (distance(upper_a, upper_b) >= config_.min_lane_segment_length_m) {
      append_lane(upper_a, upper_b);
    }
  }

  if (lanes_.empty()) {
    throw std::invalid_argument("lane exclusion leaves no usable segments");
  }
}

void LanePlanner::append_lane(const Vec2 & endpoint_a, const Vec2 & endpoint_b)
{
  lanes_.push_back(Lane{lanes_.size(), endpoint_a, endpoint_b});
}
