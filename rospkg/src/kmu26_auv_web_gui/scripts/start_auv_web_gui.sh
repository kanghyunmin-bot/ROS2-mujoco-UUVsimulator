...
ROBOT_PACKAGE="${KMU26_ROBOT_PACKAGE:-auv}"
...
if ! command -v ros2 >/dev/null 2>&1; then
  echo "[auv_web_gui] ros2 command not found."
...
if missing:
    print("[auv_web_gui] Missing Python packages: " + ", ".join(missing))
    print("Install on the Ubuntu robot PC:")
    print("  /usr/bin/python3 -m pip install --user fastapi uvicorn 'websockets>=13.0'")
    sys.exit(1)

try:
    from websockets.server import ServerProtocol  # noqa: F401
except ImportError:
    print("[auv_web_gui] Installed websockets is too old for uvicorn.")
    print("Upgrade it on the Ubuntu robot PC:")
    print("  /usr/bin/python3 -m pip install --user --upgrade 'websockets>=13.0'")
    sys.exit(1)
PY
...
if explicit:
    print(
        f"[auv_web_gui] Port {port} is already in use. "
...
for candidate in range(port + 1, port + 20):
    if is_free(candidate):
        print(
            f"[auv_web_gui] Port {port} is already in use; using {candidate} instead.",
            file=sys.stderr,
        )
        print(candidate)
        sys.exit(0)
...
print(
    f"[auv_web_gui] No free port found from {port} to {port + 19}.",
    file=sys.stderr,
)
sys.exit(1)
PY
...
echo "[auv_web_gui] Starting server"
echo "[auv_web_gui] Open from Mac: http://<ubuntu-robot-ip>:${PORT}"
echo "[auv_web_gui] Host=${HOST} Port=${PORT}"

if ros2 pkg prefix auv_web_gui >/dev/null 2>&1; then
  exec ros2 run auv_web_gui server \
    --host "${HOST}" \
    --port "${PORT}" \
...
exec python3 -m auv_web_gui.server \
  --host "${HOST}" \
  --port "${PORT}" \
  --robot-package "${ROBOT_PACKAGE}" \
  --robot-launch "${ROBOT_LAUNCH}" \
