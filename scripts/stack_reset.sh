...
SITL_PIDS="$(collect_pids 'Tools/autotest/sim_vehicle.py.*ArduSub' | xargs)"
ARDUSUB_PIDS="$(collect_pids '/build/sitl/bin/ardusub' | xargs)"
MUJOCO_PIDS="$(collect_pids 'run_urdf_full.py' | xargs)"
VIDEO_PIDS="$(collect_pids 'scripts/ros2_to_qgc_video.py' | xargs)"
MAVPROXY_PIDS="$(collect_pids 'mavproxy.py' | xargs)"
...
if [[ "${KEEP_QGC}" == "0" ]]; then
  ...
  QGC_PIDS="$(
    {
      collect_pids 'QGroundControl-x86_64.AppImage'
      collect_pids '/usr/bin/QGroundControl'
    } | sort -u | xargs
  )"
...
ps -ef | grep -E "Tools/autotest/sim_vehicle.py.*ArduSub|/build/sitl/bin/ardusub|run_urdf_full.py|mavproxy.py|QGroundControl-x86_64.AppImage|/usr/bin/QGroundControl" | grep -v grep || true
