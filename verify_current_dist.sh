...
ARCHIVE_PATH="${1:-${DIST_DIR}/dist2/ubuntu22.04/out/latest/${PACKAGE_NAME}.zip}"
...
required=(
  ...
  uuv_mujoco.zip YOLO/best.pt rospkg/README.md
...
gui_model_sha="$(sha256sum "${ROOT}/YOLO/best.pt" | awk '{print $1}')"
...
[[ "$(stat -c %s "${ROOT}/YOLO/best.pt")" -gt 1000000 ]] || fail "best.pt is unexpectedly small"
