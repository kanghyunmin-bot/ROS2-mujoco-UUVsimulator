...
git add uuv_mujoco/current rospkg/real_robot.repos
git status --short
```
...
```bash
bash -n run_control_gui.sh uuv_mujoco/current/start_sitl_mujoco_mj311.sh
python3 -m py_compile uuv_mujoco/current/run_uuv_mujoco.py
